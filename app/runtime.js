...
export const WORKER_EVENTS = Object.freeze(["phase:enter", "prompt:assembled", "tool:results"]);
...
export class Runtime {
   ...
   * @param {BrowserPorts} [options.ports]
   * @param {string} [options.workerUrl]
   * @param {string} [options.main]
   */
  constructor(options = {}) {
    /** @type {BrowserPorts} */
    this.ports = options.ports ?? browserPorts();
  ...
  /** Subscribe; the return value unsubscribes. @param {string} type @param {Listener} listener @returns {() => void} */
  on(type, listener) {
  ...
  async start() {
    ...
    try {
      ...
      const workers = this._workerUrl ?? workerUrl();
      const settings = { ports: this.ports, state: this._state, main: this.mainName, log: this.log() };
      this._main = await loadAgents({ ...settings, workerUrl: workers });
  ...
  async send(input) {
    ...
    try {
      ...
      this.emit("turn:end", {
        agent: agent.name, input, answer, at: ended,
        ms: ended.getTime() - started.getTime(), messages: agent.messages,
      });
      return answer;
    } catch (error) {
  ...
  async startCron() {
    this._stopCron = await this.ports.cron.start({
      ...
      onMissed: (missed) => {
        for (const one of missed) this.emit("error", { kind: "missed-job", name: one.job.name, runs: one.runs, since: one.since });
      },
    });
  }
