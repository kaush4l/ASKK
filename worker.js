// ASKK VM worker (unit 2) — copy-adapted from Eliza docs/worker.js.
// The protocol with worker-util.js / wasi-util.js (unit 3, not in this
// worktree) is Eliza's, verbatim: serveIfInitMsg / getImagedata /
// getImagename / recvCert / getCertDir / wasiHackSocket /
// sockWaitForReadable / errStatus. Do not rename.
// TtyClient comes from the vendored xterm-pty@0.9.4 workerTools bundle
// (unit 5 vendors it; path mirrors the package layout).
...
var dbgc = new BroadcastChannel("askk-dbg");
            ...
            if (netParam && netParam.mode == 'browser') {
                ...
                recvCert().then((cert) => {
                    ...
                    env = [
                        ...
                        "HTTPS_PROXY=http://192.168.127.253:80",
                        "HTTP_PROXY=http://192.168.127.253:80",
                        // ASKK: fixed sentinel URLs — the browser-side net stack
                        // (stack.js, unit 3) rewrites them to the real targets.
                        // Loopback or real hostnames must never appear here: the
                        // guest's no_proxy would shadow them.
                        "ASKK_MODEL_URL=http://llm.askk.internal/v1",
                        "ASKK_MODEL_NAME=" + (getQueryParam('model') || 'gemma-4-12B-it-qat-mxfp8'),
                        "ASKK_PERSIST_URL=http://persist.askk.internal/__persist",
                        "ASKK_INGRESS_URL=http://ingress.askk.internal"
                    ];
                    listenfd = 4;
...
function startWasi(wasm, ttyClient, args, env, fds, listenfd, connfd) {
    ...
    // apart from a hung one (visible via the askk-dbg BroadcastChannel).
    ...
    var hb = { clock: 0, poll: 0, write: 0, acc: 0, accok: 0, snd: 0, rcv: 0, rcvok: 0, last: Date.now() };
    var hbTick = function () {
        var now = Date.now();
        if (now - hb.last >= 10000) {
            hb.last = now;
            dbg("vm heartbeat: clock=" + hb.clock + " poll=" + hb.poll + " write=" + hb.write +
                " acc=" + hb.acc + "/" + hb.accok + " snd=" + hb.snd + " rcv=" + hb.rcv + "/" + hb.rcvok);
        }
    };
    ...
    var _hbAcc = wasi.wasiImport.sock_accept;
    wasi.wasiImport.sock_accept = function () {
        hb.acc++; if (hb.acc === 1) dbg("worker: first sock_accept");
        var r = _hbAcc.apply(wasi.wasiImport, arguments);
        if (r === 0) { hb.accok++; if (hb.accok === 1) dbg("worker: sock_accept OK (proxy connected)"); }
        hbTick(); return r;
    };
    var _hbSnd = wasi.wasiImport.sock_send;
    wasi.wasiImport.sock_send = function () {
        hb.snd++; if (hb.snd === 1) dbg("worker: first sock_send (guest TX)");
        hbTick(); return _hbSnd.apply(wasi.wasiImport, arguments);
    };
    var _hbRcv = wasi.wasiImport.sock_recv;
    wasi.wasiImport.sock_recv = function () {
        hb.rcv++;
        var r = _hbRcv.apply(wasi.wasiImport, arguments);
        if (r === 0) { hb.rcvok++; if (hb.rcvok === 1) dbg("worker: first sock_recv OK (proxy RX)"); }
        hbTick(); return r;
    };
