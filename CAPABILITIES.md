...
| Terminate a runaway thread | — | `pool.stop(id)`, reachable from the status line | have | have | unverified | — | **The method with no caller is still there and is still not the answer.** `terminate()` kills every thread in the pool, which is a shutdown; what was missing was ending ONE. `AgentWorkerPool.stop(id)` terminates the worker carrying that task, settles every pending call it owed so no caller waits forever, and marks the record `stopped` — a fourth `TaskState`, not a flavour of `failed`, because a run the user ended is not a bug. Six tests in `test/backend/AgentWorkerPool.test.js` drive it through a fake thread: the thread is terminated, another agent's thread is untouched, a second task on the same thread is told it died with it, a finished task cannot be stopped, and an unknown id answers `false` rather than throwing. Two more in `test/backend/AgentService.test.js` cover the `agents.stop` route, including a build with no pool. The page reaches it from the one place the work is mentioned: `statusLine` now returns `stoppable` beside "researcher is working in the background", and `Header` draws a `stop researcher` button from it (`test/app/phrasing.test.js`, four tests, one of which pins the defect that a `stopped` task used to announce itself as "has an answer for you"). `bun run lint` clean over 186 files, `bun test` 950 pass, `bun run build` and `bun scripts/smoke.js` green. **The press itself is driven in a real browser.** `scripts/smoke.js` types a question whose handed-over call the scripted endpoint holds open for 60 s — a thread worth stopping is a busy one, and a busy thread is exactly the one that cannot read a polite request — waits for the control to appear, clicks it, and asserts what the status line says next: *"stop researcher" pressed while it said "researcher is working in the background", after which it said "researcher was stopped" and the control went away.* The delay is scoped to the sub-agent's prompt by the absence of `check_task`, because matching the task text alone also matched the parent quoting it and held the page's own turn open. `unverified` on iOS only, like every row here: no measurement was taken there |
...
| A named strategy library (plan-then-execute, write-critique-improve, delegate) | deepseek `packages/workflow/tool-ralph/README.md:12` | none | absent | absent | absent | — | `engine/index.js:6-8` — one loop; `tools/index.js:28-58` — seven built-in tools. `plan` is the nearest thing and is deliberately not a strategy: it records a decomposition the model wrote, and nothing reads it back to decide how to work |
...
| A goal that outlives the turn | Hermes `delegate_task(goal, …)` — the first argument | a field on the conversation, restated in every prompt | have | have | unverified | — | `Conversation.aim` / `Conversation.goal`, written through `ConversationService.aim` and rendered as the `goal` block (`Engine.blocks`, `PromptTemplate.DEFAULT_ORDER`). Set from the drawer's goal field; survives a reload because it is on the stored record. Volatile by placement on purpose — it sits after `context`, so editing it costs its own length rather than the whole cacheable prefix |
| That goal composed into tasks | Open SWE's plan step; Manus's recitation | a list of steps with states, written and revised by the agent | have | have | unverified | — | `core/Plan.js` + `core/tools/PlanTool.js` + `core/tools/PlanPort.js`, named in `agents/main/agent.md`'s `tools:`, rendered as the `plan` block beside the goal. Measured 2026-09-17 through the built page (`bun run smoke`): a goal was typed into the drawer, the agent wrote three steps without being told how many, ticked the first off, and the panel showed the list WHILE the turn was still running — which only `EventName.PLAN` can do, because the conversation row is otherwise re-read on open. Persisted through `ConversationService.compose`; `test/backend/PlanThroughTheLoop.test.js` drives a plan written on one turn into the prompt of a later one |
| The agent reading its own plan back to decide what to do | — (no reference ships a loop that branches on one) | nothing reads it | absent | absent | absent | — | The plan is recitation, and that is the whole claim: it re-enters the prompt every turn and the MODEL decides what to do with it. `grep -rn "outstanding" src/core/engine` is empty — no loop consults a step's state, refuses to answer with steps left, or picks the next one. Deliberate for now — the dissent `ROADMAP.md` records under the cluster item is that one agent reading a plan may do the work a three-agent cluster was proposed for, and a loop that branched on step states would settle that question before it had been asked |
...
| Work that outlives the turn that started it | claw-code `task.ts`; pi `subagent` (both process-backed) | `wait: false`, a task id, and a line in the next turn's context | have | unverified | unverified | — | `AgentWorkerPool.start` + `core/tools/TasksPort.js` + `check_task`; measured 2026-09-02 through the built page over two typed turns |
| Work that outlives the TAB it was handed over in | Devin's devbox (a persistent process); claw-code and pi both process-backed | the record is stored, and the question is put on a new thread when the page reopens | have | have | unverified | — | **This cell used to read "in memory only: a record that survived a reload would describe work that does not exist", and the premise was wrong rather than the conclusion.** A sub-agent keeps no transcript and `agentWorker.js` builds a fresh agent per message, so the instruction IS the whole of a run's state and running it again is not an approximation of resuming it. `AgentWorkerPool.resume` + `STORE_TASKS` (`composition.js`, database version 5) + `TaskState.INTERRUPTED`. Measured 2026-09-17 through the built page (`bun run smoke`): a question was handed over against an endpoint that never answers it, the page was reloaded, and the new tab's rail said *"researcher is working in the background, restarted after the tab closed"* with nothing typed into it. Bounded on purpose — an hour of staleness (`STALE_AFTER`) and three restarts (`MAX_RESUMES`), past either of which the record is kept, shown, and left alone rather than silently spending a user's tokens on yesterday's question |
