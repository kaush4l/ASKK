...
| An agent that checks its own work before finishing | — | one declared tool call, run once, judged by the agent | yes | unverified | unverified | — | `check:` in an agent file, `ReActEngine` runs it at the answer branch and hands the output back rather than reading pass or fail out of it |
