...
1. **An empty evidence cell makes the status `unverified`**, whatever we believe.
   ...
   next one on the list takes its place: multimodal is unreachable
...
| A sub-agent that reports progress before it finishes | claw-code `task.ts` (a child's events are forwarded to the parent's stream) | **none** | absent | absent | absent | — | `AgentWorkerPool.ask` awaits one message and `SubAgentTool.call` awaits one promise; the pool records `startedAt` and `calls` and nothing about what a run is doing. A four-minute delegated run is four minutes of silence in a view that streams the parent's every token |
...
| Capability | Reference | Ours | Chr | Saf | iOS | Root | Evidence |
|---|---|---|---|---|---|---|---|
| Tool calls as they happen | elizaOS `task-activity-store.ts` | the call, never the result | degraded | degraded | unverified | — | `ReActEngine.js:275` `onStep?.(…)` emits the step before `:390` runs it; `ChatService.js:273-277` sends `{step, answer, isAnswer, thinking}` and **no observation**, which is the whole reason this is `degraded`. Rendered live at `page.jsx:565`, whose anchor is ``data-testid={`step-${taken.step}`}``, and kept after the turn at `RunPanel.jsx:55`, anchor ``data-testid={`run-step-${taken.step}`}``. Seen by the accountant, 2026-09-01: `step 1shell({"command": "python3 receipt.py; python3 -V"})` on screen while the guest was still running it. `docs/LEDGER.md` row S48 closed |
...
| Viewing and editing a file | bolt.diy `CodeMirrorEditor.tsx` | viewing, coloured, read-only | degraded | unverified | unverified | — | **Half of it, on purpose and at a priced refusal.** Viewing is `have`: `FilesPanel.jsx` renders the bytes through `src/client/highlight.js`, a hand-written scanner covering c, js, json, md, py, sh, which said `data-language="py"` and 3 coloured runs over the file the agent had just written, and names the language it cannot place (`data-testid="file-unknown-language"`). Editing is `absent`: `read-only` is on screen at `FilesPanel.jsx:173` and there is no writer. `grep -rni "codemirror\|monaco" package.json src` → no matches, and the refusal is measured rather than asserted — the minimal CodeMirror is 366,728 raw / +29.8% of a cold load's gzipped JS+CSS, against a highlighter that is one file |
