...
| prompt assembly and order | **genuine.** `Engine.blocks` / `Engine.assemble` over `PromptTemplate` `DEFAULT_ORDER`. `test/bench/oursScaffold.test.js` asserts the message sent is `engine.assemble(...).text`, re-derived, not a pinned string. |
