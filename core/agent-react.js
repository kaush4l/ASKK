...
export async function reactLoop(agent, phaseComponents = null) {
  ...
  while (!isAnswer(parsed)) parsed = await reactStep(agent, phaseComponents)
...
async function callTools(agent, parsed) {
  ...
  const observation =
    seen > 1
      ...
      : await agent.toolbox.invoke(call, (results) => report(agent, results))

  agent.transcript.add("user", `Result: ${observation}`)
  return parsed
}

/** @param {Agent} agent @param {{ tool: string, ok: boolean }[]} results @returns {void} */
