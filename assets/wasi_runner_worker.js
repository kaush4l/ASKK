...
`);
  ...
  function seedWorkspace(files, mounts) {
    ...
    for (const mount of Array.isArray(mounts) ? mounts : []) {
      ...
      if (path === null || !(bytes instanceof Uint8Array))
        continue;
  ...
  function collectChanged(dirMap, seeded, prefix, out, reserved) {
    for (const [name, inode] of dirMap.entries()) {
      ...
      if (prefix === "" && reserved && reserved.has(name))
        continue;
  ...
  self.onmessage = async (event) => {
    ...
    try {
      const cacheKey = typeof request.cache_key === "string" && request.cache_key !== "" ? request.cache_key : null;
      let wasmBytes;
      let mounts;
      try {
