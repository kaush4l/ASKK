...
### Process-backed servers (via the local bridge)

The two kinds above run entirely in the tab. The third kind, **process-backed servers**
(`McpServerKind::Process`), runs a real **stdio MCP server** — any off-the-shelf server
distributed as a command, like `npx chrome-devtools-mcp@latest`. The browser cannot spawn
a process, so these **require the [local bridge](#bridge-mode-real-bunnode-on-disk) running
with `--allow-exec`**: the bridge spawns the child, relays JSON-RPC to it over HTTP, and
correlates responses back to the tab. With no bridge (or no `--allow-exec`), process servers
simply do not start; module and shellized servers are unaffected.

The server config is a JSON spec stored in the server's `definition` field — `command`,
optional `args`, `env` (an array of `[key, value]` pairs), and `cwd` (relative to the bridge
run root, or `null` for the run root itself). For a Chrome DevTools MCP server:

```json
{ "command": "npx", "args": ["chrome-devtools-mcp@latest"], "env": [], "cwd": null }
```

Only an allow-listed binary may be spawned (`npx`, `npm`, `node`, `bunx`, `bun`, `pnpm`,
`yarn`, `deno`, …); anything else is rejected by the bridge.

**Recipe — wire up Chrome DevTools MCP:**

1. Start the bridge with execution enabled (see [Bridge mode](#bridge-mode-real-bunnode-on-disk)):

   ```sh
   node scripts/askk-local-bridge.mjs --allow-exec
   ```

2. Open the **MCP** dashboard page → **Add process server**.
3. Paste the config above into the JSON editor and save.
4. Click **Discover tools**. The bridge spawns `npx chrome-devtools-mcp@latest`, runs the
   `initialize` → `tools/list` handshake, and the discovered tools appear (e.g.
   `navigate_page`, `take_snapshot`, `click`).
5. The agent can now call those tools on any run, exactly like a built-in or in-browser MCP
   tool — each `tools/call` is relayed through the bridge to the child process.

**Security note.** `--allow-exec` lets the agent spawn **real processes** on the bridge
machine — only run it on a machine you control, and remember that the command binary must be
in the bridge's allow-list (above). Process servers are the only MCP kind that can reach
outside the tab; the default is exec **disabled**.

#### Bridge relay routes and curl smoke test

The bridge exposes three relay routes (all `POST`, JSON in/out, gated by `--allow-exec`):

- `POST /askk/mcp/spawn` — body `{ id, command, args?, env?, cwd? }`. `env` is an array of
  `[key, value]` pairs; `cwd` resolves under the run root. Returns `{ "success": true,
  "data": { "id": <id> } }`. Re-spawning the same `id` tears down the previous child first.
- `POST /askk/mcp/send` — body `{ id, message }` where `message` is a JSON-RPC object. For a
  **request** (carries an `id`), returns `{ "success": true, "data": <JSON-RPC response> }`.
  For a **notification** (no `id`, or `method` starting with `notifications/`), the frame is
  written to the child and the route returns `{ "success": true, "data": null }` without
  awaiting a reply.
- `POST /askk/mcp/kill` — body `{ id }`. Returns `{ "success": true }`. Failures on any route
  return `{ "success": false, "error": <string> }` with a `400`.

Stdio framing to the child is **newline-delimited JSON-RPC 2.0** (`"2024-11-05"` protocol);
responses correlate by JSON-RPC `id`.

You can prove the relay end to end without Chrome using the bundled, dependency-free stub
stdio server, [`scripts/mcp-stdio-stub.mjs`](scripts/mcp-stdio-stub.mjs) (it answers
`initialize`, `tools/list` with one `echo` tool, and `tools/call`). Start the bridge with
`node scripts/askk-local-bridge.mjs --allow-exec`, then — `args` paths resolve against the
run root, so pass an **absolute** path to the stub:

```sh
STUB="$(pwd)/scripts/mcp-stdio-stub.mjs"

# 1. spawn the stub stdio server
curl -s -X POST http://127.0.0.1:8874/askk/mcp/spawn \
  -H 'Content-Type: application/json' \
  -d "{\"id\":\"stub-1\",\"command\":\"node\",\"args\":[\"$STUB\"]}"
# -> {"success":true,"data":{"id":"stub-1"}}

# 2. initialize
curl -s -X POST http://127.0.0.1:8874/askk/mcp/send \
  -H 'Content-Type: application/json' \
  -d '{"id":"stub-1","message":{"jsonrpc":"2.0","id":1,"method":"initialize",
       "params":{"protocolVersion":"2024-11-05","capabilities":{},
       "clientInfo":{"name":"curl","version":"0"}}}}'
# -> {"success":true,"data":{"jsonrpc":"2.0","id":1,
#      "result":{"protocolVersion":"2024-11-05", ... "serverInfo":{...}}}}

# 3. notifications/initialized (notification: no reply, data is null)
curl -s -X POST http://127.0.0.1:8874/askk/mcp/send \
  -H 'Content-Type: application/json' \
  -d '{"id":"stub-1","message":{"jsonrpc":"2.0","method":"notifications/initialized"}}'
# -> {"success":true,"data":null}

# 4. tools/list
curl -s -X POST http://127.0.0.1:8874/askk/mcp/send \
  -H 'Content-Type: application/json' \
  -d '{"id":"stub-1","message":{"jsonrpc":"2.0","id":2,"method":"tools/list"}}'
# -> {"success":true,"data":{"jsonrpc":"2.0","id":2,
#      "result":{"tools":[{"name":"echo", ...}]}}}

# 5. tools/call echo
curl -s -X POST http://127.0.0.1:8874/askk/mcp/send \
  -H 'Content-Type: application/json' \
  -d '{"id":"stub-1","message":{"jsonrpc":"2.0","id":3,"method":"tools/call",
       "params":{"name":"echo","arguments":{"text":"hello relay"}}}}'
# -> {"success":true,"data":{"jsonrpc":"2.0","id":3,
#      "result":{"content":[{"type":"text","text":"hello relay"}]}}}

# 6. kill
curl -s -X POST http://127.0.0.1:8874/askk/mcp/kill \
  -H 'Content-Type: application/json' \
  -d '{"id":"stub-1"}'
# -> {"success":true}
```
