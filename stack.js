// ASKK net stack — main-thread bridge between the VM worker and the
// c2w-net-proxy stack worker. Vendored from Eliza (kaush4l/Eliza docs/stack.js).
// ASKK adaptations:
//   - window.AskkNet.attach(vmWorker, getBackend) wraps Eliza's inline newStack
//     wiring (boot.js owns VM worker creation + image loading; attach owns the
//     SAB channels, the stack worker, and the sentinel remap).
//   - askkRemapURL replaces Eliza's window.__guestURLMap.
// SAB layout per channel (protocol law, see CONTRACTS.md): 12 + 4096 bytes —
// streamCtrl@0 streamStatus@4 streamLen@8 data@12.

// Pure sentinel remap: guest-visible URLs -> browser-side targets.
//   http://llm.askk.internal/v1      -> backend (user's OpenAI-compatible base
//                                       URL, already ending in /v1; read per
//                                       request so settings changes apply live)
//   http://persist.askk.internal/... -> same-origin path resolved against
//   http://ingress.askk.internal/... -> `origin` (the service worker intercepts)
//   http://bin.askk.internal/<name>  -> same-origin ./bin/<name> — the public
//                                       binary shelf the guest pulls tools
//                                       from (askk-get)
// Non-sentinel URLs pass through untouched.
// `backend` is the base-URL string; the page-level getBackend() returns
// {url, model} (index.html contract), so an object is unwrapped to .url —
// String({...}) would otherwise remap llm traffic to "[object Object]/…".
function askkRemapURL(url, backend, origin) {
    if (backend && typeof backend === "object") backend = backend.url || "";
    // A sentinel prefix only matches at a URL boundary ("/", "?", "#" or end),
    // so e.g. llm.askk.internal.evil.com or /v1x never remap.
    function boundary(rest) {
        return rest === "" || rest[0] === "/" || rest[0] === "?" || rest[0] === "#";
    }
    var LLM = "http://llm.askk.internal/v1";
    if (url.startsWith(LLM) && boundary(url.slice(LLM.length))) {
        return String(backend).replace(/\/+$/, "") + url.slice(LLM.length);
    }
    var BIN = "http://bin.askk.internal";
    var binRest = url.slice(BIN.length);
    if (url.startsWith(BIN) && boundary(binRest)) {
        return new URL("./bin" + (binRest === "" ? "/" : binRest), origin).href;
    }
    var SAME_ORIGIN = ["http://persist.askk.internal", "http://ingress.askk.internal"];
    for (var i = 0; i < SAME_ORIGIN.length; i++) {
        var rest = url.slice(SAME_ORIGIN[i].length);
        if (url.startsWith(SAME_ORIGIN[i]) && boundary(rest)) {
            // "/__persist/x" -> "./__persist/x" resolved against the page URL,
            // so the site works under a subpath (GitHub Pages).
            return new URL("." + (rest === "" ? "/" : rest), origin).href;
        }
    }
    return url;
}

// Wires the fetch-proxy network stack onto an already-created VM worker:
// creates both SAB channels, spawns the stack worker (which boots
// ./vendor/c2w-net-proxy.wasm), and posts the VM-side SAB to vmWorker as an
// {type:"init", buf} message. Returns the VM-side message handler for boot.js
// to pass to TtyServer.start(vmWorker, handler) (Eliza's nwStack).
function attach(vmWorker, getBackend) {
    let p2vbuf = {
    ...
    var stackWorker = new Worker("./stack-worker.js");
    var proxyShared = new SharedArrayBuffer(12 + 4096);
    stackWorker.onmessage = connect("proxy", proxyShared, proxyConn, certbuf, getBackend);
    ...
    return connect("vm", vmShared, vmConn, certbuf, getBackend);
}

function connect(name, shared, conn, certbuf, getBackend) {
    ...
    return function(msg){
        ...
        if (typeof req_ == "object" && req_.type) {
            ...
            case "http_send":
                ...
                // Guest-visible sentinel URLs (llm.askk.internal, …) map to
                // real targets the page can fetch. getBackend() is read per
                // request so backend settings changes apply live.
                var addr = askkRemapURL(
...
if (typeof window !== "undefined") {
    window.AskkNet = { attach: attach, askkRemapURL: askkRemapURL };
}
