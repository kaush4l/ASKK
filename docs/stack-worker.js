// Vendored from Eliza (kaush4l/Eliza docs/stack-worker.js).
// ASKK adaptation: the proxy image lives at ./vendor/c2w-net-proxy.wasm
// (fetched relative to this worker script), so no imagename init field needed.
...
// Progress markers on the shared debug bus — the proxy is otherwise a black
// box and its silent death looks identical to a wedged guest handshake.
var sdbg = (m) => { try { new BroadcastChannel("askk-dbg").postMessage(Date.now() + " stack: " + m); } catch (e) {} };
sdbg("worker script loaded");
self.onerror = (e) => sdbg("worker error: " + (e && e.message || e));

onmessage = (msg) => {
    sdbg("init message received");
    serveIfInitMsg(msg);
    var fds = [
        undefined, // 0: stdin
        undefined, // 1: stdout
    ...
    fetch("./vendor/c2w-net-proxy.wasm", { credentials: 'same-origin' }).then((resp) => {
        sdbg("proxy wasm fetch: " + resp.status);
        resp['arrayBuffer']().then((wasm) => {
            sdbg("proxy wasm bytes: " + wasm.byteLength);
            WebAssembly.instantiate(wasm, {
                "wasi_snapshot_preview1": wasi.wasiImport,
                "env": envHack(wasi),
            }).then((inst) => {
                sdbg("proxy instantiated, starting");
                try {
                    wasi.start(inst.instance);
                    sdbg("proxy EXITED (unexpected)");
                } catch (e) {
                    sdbg("proxy CRASH: " + (e && e.message || e));
                    throw e;
                }
            }, (e) => sdbg("proxy instantiate FAILED: " + (e && e.message || e)));
        })
    }, (e) => sdbg("proxy fetch FAILED: " + (e && e.message || e)));
};
