...
## 6.1 — Design law + tokens: `checks/design.ts` in the gate, one named sub-check per rule — 2026-08-28
- Files: `src/ui/tokens.css` (new), `scripts/checks/design.ts` (new), `scripts/gate.ts` (adds the `design` check, removes it from `SCHEDULED`), `docs/PROGRESS.md`
- Proof: `bun run gate` → **gate GREEN, 7 checks ran, 0 failed** (was 6; `design` is the seventh). `design: 8 sub-check(s), 0 failed, 2 pending (frontdoor-copy, addresses)`. `gate-coverage: 4 check(s) on disk in scripts/checks, 4 named by scripts/gate.ts`. Every sub-check was **watched red** with a planted violation and reverted:
  1. *`scan-roots`* — `mv src/ui/tokens.css` away → `src/ui holds no .css/.ts/.tsx file — a root that matches nothing is a broken check, never a clean tree`. This is the sub-check that exists because `src/ui/**` was empty when the increment started and every other rule here is a filter over the same list.
  2. *`tokens`* — a planted `src/ui/plant.css` with `#ff0000`, `rgb(`, `13px`, `200ms` → four failures, each naming file, line, the literal and the token to use instead.
  3. *`ramp`* — `font-size: var(--nope)` and `font-family: Helvetica` → two failures; an eighth step `--t-caption: 400 9px/12px` → *"an eighth type step — DESIGN §3.4 says 'there are no others'"* **and** *"sets --t-caption to 9px — DESIGN §7 floors text at 11px"*. The eighth-step half caught its own author first: the file shipped a `--t-tabular` token and the check went red on it before any plant was written.
  4. *`motion`* — deleting `--m-quick` from the reduced-motion block → *"declares --m-quick but the prefers-reduced-motion block does not override it"*; setting `--m-settle: 0ms` → *"reduces --m-settle to `0ms`, not 0.01ms … a zero-duration transition may never fire transitionend"*; `transition: color 200ms ease` in a scanned file → the literal-duration half.
  5. *`contrast`* — `--ink-3` dimmed to `#6C6660` → *"dark: --ink-3 measures 2.89:1 at worst, under the 4.5:1 text floor"* **and** *"docs/DESIGN.md §3 records 5.34:1 — the palette and the law disagree about one number"*; `--line-strong` lightened in the light theme → the 3:1 non-text failure and its DESIGN row; changing one copy of a theme only → *"declares --bg as `#F5F1EA` in :root:not([data-theme="dark"]) and `#EEE9E0` in [data-theme="light"] — one theme, two declarations, drifted"*.
  6. *`fonts`* — a Google Fonts `@import` and `src: url(/fonts/plex.woff2)` → the CDN failure and two absolute-url failures. A relative `url("./fonts/plex.woff2")` inside `@font-face` stays green, which is the correct answer under `basePath`.
  7. *`frontdoor-copy`* — a planted `src/ui/surfaces/Door.tsx` with 33 words of JSX prose → *"carries 33 words of prose against DESIGN §4.1's cap of 22"*.
  8. *`addresses`* — a planted `src/ui/shell/surfaces.ts` with a duplicate `?panel=workbench` and a `?surface=context` → three failures; an empty registry → *"declares no { id, address } entry this check can read — either the registry moved or its shape changed"*.
- Ringmaster: not yet ruled
- Open:
  - **`frontdoor-copy` and `addresses` report PENDING, not ok.** Their subjects (`src/ui/surfaces/Door.tsx`, `src/ui/shell/surfaces.ts`) are 6.2's files and were outside this increment's ownership. Both were watched red against planted subjects and both run for real the moment those files land. PENDING is printed with the file it waits for and the increment that writes it; it is not silence and it is not a pass.
  - **`DESIGN.md` was not edited, and needed no edit.** PLAN's own note is correct. Two things in it are *observations, not defects*, and both are present in its first commit `711a958`, so neither is drift: §3.3 says *"Two tinted fills exist for chips"* above a table with one row (`--live-fill`, with a dark and a light value — the "two" may mean the two themes); and §5's local path says *"Lands on `/work`"* while §4 rules addresses are `?panel=<id>`, so the cold-open step reads as a path where the law says it is an address. Raised to the owner rather than edited, per §10.2's convention.
  - **DESIGN §3's measured tables were recomputed, not trusted.** All eight recorded rows reproduce exactly from the token file: `--ink` 14.54/14.13, `--ink-2` 7.87/7.58, `--ink-3` **5.34/5.27**, `--line-strong` 3.12/3.20, `--live` 7.79/4.73, `--ok` 6.82/5.14, `--fail` 5.24/5.30, `--attn` 7.17/5.56. Focus ring `--live` on `--bg` 8.90/5.26. `--ink` on `--live-fill` 12.27/14.15. The check now recomputes them on every gate run, so the numbers are a result rather than a sentence.
  - **`--figures-tabular`, `--t-display-tracking` and `--t-micro-tracking` are tokens DESIGN describes as properties of a step rather than naming as tokens** (§3.4: *"`--t-meta` and `--t-display` set `font-variant-numeric: tabular-nums`"*, and the two tracked steps). They exist because the `font:` shorthand cannot carry letter-spacing or numeric variant, and without them a builder would have to write `0.06em` raw in a file the `tokens` sub-check governs. No new visual value was invented: every number is DESIGN's.
  - **`tokens.css` has no importer yet.** 6.2 wires it into the shell; this increment did not touch `src/app/**`. The check reads the file directly, so the rules are armed before the stylesheet is loaded — but the *rendered* consequence of these tokens is unmeasured until `scripts/browser/contrast.ts` seeds `contrast-ratchet.json` at 6.2, exactly as DESIGN's own VERDICT says.
