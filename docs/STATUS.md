...
| 9 | One typed reader for tool arguments (`context::Args`); `crates/script` + `agent::forge` deleted out of the shipping graph | COMMITTED main `3033672`, NOT PUBLISHED — `name` trims/refuses blank, `text` is verbatim; parsing is total; `cargo tree -p core -i rhai` no longer resolves; ADR-003 records what survives |
| 10 | The measurement round: four owner claims executed, three false | COMMITTED main `e27a387`, NOT PUBLISHED — adding an agent took FOUR files not one; two `role: entry` agents were resolved by ALPHABETICAL ORDER (now reported and the loser stripped); the route parser dropped `**ROUTE:**`/bullet/numbered shapes while a test COMMENT claimed otherwise (criteria moved to `public/stages/strategy.md`); the log stops keeping secrets |
| 11 | The environment stops losing work: `read_range`, a checked `edit_file`, a non-interactive guest boot | COMMITTED main `0a99e9f`, NOT PUBLISHED — two of four acceptance bullets were already satisfied at `e27a387` and were re-verified rather than rebuilt; the roadmap specified the window on `dd`/`sed -n`, neither of which is in `BINARIES` |
| 12 | The route becomes a fact the core owns: `board/flow.rs`, five `data-*` attributes, `Route::named` | COMMITTED main `2f3af83`, NOT PUBLISHED — `ROUTE_CHOSEN` is NOT a loop fact and that non-change is the ruling; `stage.rs` 158 -> 127, `shell()` exactly 40, baseline unchanged at 52 |
| 13 | The budget round: the shipped agent was being shown NO `## observations` at all | COMMITTED main `5131e0b`, NOT PUBLISHED — live at HEAD, not latent: 4174 tokens wanted against a 4096 budget before a conversation existed. `WORK_BUDGET` 4096 -> 8192 **PROVISIONAL (§17), the owner's measurement is outstanding**. The response contract wins over the compaction notice; `edit_file` granted and reachable |
| 14 | The surface: 11 hover rules gated off coarse pointers, `docs/DESIGN-SYSTEM.md` written | COMMITTED main `d5b1cb0`, NOT PUBLISHED — 0 of 11 hover rules reachable at 375px, 11 of 11 at 1280px; `web/` 1802 -> 1793 lines; three roadmap CSS measurements corrected |
| 15 | The P0: a deploy bricked the live page, and the gate grew an instrument that can see it | COMMITTED main, NOT PUBLISHED — `fetch(request)` was answered by the HTTP cache, so a wasm-bindgen snippet at a CRATE-hash path paired new `index.html` with old bytes and SRI blocked the bundle. One word, `{cache:"reload"}`. Plus `scripts/sw-cache-probe.py` (two scenarios), `check-url-churn.py` in check 6, `canary-live.py` beside it, and `web/c2w/dist/` TRACKED — every deploy ever had depended on three untracked files on one machine |
| 16 | An eviction reaches the log; three lists stop lying | COMMITTED main `9fe9542`, NOT PUBLISHED — `ModelCalled.evicted`, elisions only; `write_agent` named 5 workspace tools of 11; `PhaseId::Verify`'s budget deleted (its exit led to a `Plan` config that never existed and would have panicked) |
| 17 | An artifact becomes a type, addressable and cross-thread | COMMITTED main `b1c8abb`, NOT PUBLISHED — `struct Artifact` where `grep` found none; words gated on the port's real answer rather than a second copy of every deliverable; `read_artifact` built on increment 11's `read_range`. Cross-thread proved with two `IdbStore` handles, NOT two Workers — stated, and short of the criterion |
| 18 | The router's grammar is a stated rule; the phase machine that never ran is gone | COMMITTED main, NOT PUBLISHED — CommonMark block prefixes IN, the edge measured in both directions; `Verdict`/`PlanSteps`/`App.phases` deleted, `AgentState.plan` LABELLED-and-kept because deleting it breaks resume of stored state. `docs/ROADMAP.md` re-measured against 24 commands |
| 19 | The Flow Rail, and two UI gates that were reading the wrong pixel | COMMITTED main, NOT PUBLISHED — route/walk/lap from typed reads, no `Route` enum in `crates/ui`; `layout-audit.js` now recurses into `CSSMediaRule` (594 assertions had been reading the resting paint); the 44px floor is a gate with `.skip-link` exempt BY NAME |
| 20 | The boot fallback stops prescribing a reload that cannot work | COMMITTED main, NOT PUBLISHED — verified against the shipped `dist/index.html` with one `integrity` corrupted; the proposed listener's expectation was WRONG (two events, the inline module first with no `href`), caught before landing. NOT GATED and says so |
