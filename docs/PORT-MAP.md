...
| `agents/<name>/tools.py` | a page cannot import and execute a Python module. Agent-owned tools become `agents/<name>/tools.js` ES modules. The fs port is what *finds* them — `core/agentfile-tools.js` asks it whether the file is there — but never what runs them: see the row below. |
| a `tools.js` the user writes into OPFS | a browser cannot `import()` a path inside OPFS; there is no URL for one, and the fs port hands back text, which is not a module. So a tool module that ships **with** the page is bundled and `app/worker.js` resolves it by path, already bound to the ports (`cronTools` needs `{ cron, fs, launch }`, and an agent folder can reach none of them). `app/seed.js` writes the same bytes into the workspace so the source of an agent's tools is visible there, but **editing that copy changes nothing until the page is rebuilt**. Serving a user-written module would need either a blob URL built from the text — which imports nothing relative, so `../../core/schedule.js` would not resolve — or an interpreter, and this build ships neither. |
