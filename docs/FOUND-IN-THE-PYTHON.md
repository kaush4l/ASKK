...
| D-3 | `utils.py:85` `parse_agent_file` | `yaml.safe_load(frontmatter) or {}` swallows frontmatter that is exactly `false`, `0`, `''` or `~` into `{}` — the same silent-empty-config failure the guard on the next line exists to prevent | those reach an error, and it is the guard's own message: `frontmatter must be a YAML mapping, got boolean` / `number` / `string` / `null`. The type names are JavaScript's where `utils.py:86-87` printed Python's (`str`, `int`, `bool`), because they name what this parser actually built; a person reads this string and no model does. `core/frontmatter.js` `bareValue` is where the first line is judged, and a line that is a key still reaches the ordinary parse and its `expected 'key: value'` |
