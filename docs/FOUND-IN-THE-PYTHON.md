...
|---|---|---|---|
| P-1 | `tools.py:98` `Tool.from_engine` | the goal rescue does `str(value or "")` over every extra argument, so `{"task": {"goal": "x"}}` starts a sub-agent on the string `{'goal': 'x'}` | changing it changes what a sub-agent receives; that is a ruling, not a port |
| P-2 | `tools.py:267` `Toolbox.invoke` | the no-calls-found message renders a **list repr** when `text` is a list, because `parse_batches` joins first and the error message does not | the model reads this string; the byte rule forbids improving it |
