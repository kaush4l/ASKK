...
mod imp {
    ...
    use askk_engine::state::{BlobStore, KvStore, LocalBoxFuture, StoreError};
...
pub(super) async fn stores() -> Result<
    (
        std::rc::Rc<dyn askk_engine::state::KvStore>,
        std::rc::Rc<dyn askk_engine::state::BlobStore>,
    ),
    String,
> {
    use askk_engine::state::{BlobStore, KvStore};
    use std::rc::Rc;
...
impl OpfsKv {
    pub async fn new() -> Result<Self, askk_engine::state::StoreError> {
        panic!("OpfsKv is wasm-only (OPFS); host runs use askk_engine::state::MemKv")
    }
}
...
impl OpfsBlob {
    pub async fn new() -> Result<Self, askk_engine::state::StoreError> {
        panic!("OpfsBlob is wasm-only (OPFS); host runs use askk_engine::state::MemBlob")
    }
}
