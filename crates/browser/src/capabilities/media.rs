...
//! returning — no stream outlives its call. Backs the Features lab's "capture"
//! buttons; nothing here is an agent tool.
...
/// One captured still frame, both as raw PNG bytes and as a `data:` URL (for
/// inline preview in the lab).
#[derive(Debug, Clone)]
...
/// honors this with a fresh user gesture, so it can fail with `NotAllowedError`;
/// that error is surfaced as-is.
#[cfg(target_arch = "wasm32")]
...
pub async fn record_microphone(seconds: f64) -> Result<CapturedAudio, String> {
    ...
    let (stopped_tx, stopped_rx) = futures::channel::oneshot::channel::<()>();
