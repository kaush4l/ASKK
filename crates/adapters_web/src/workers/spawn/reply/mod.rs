...
pub(crate) use channels::{Activity, Authored, Memory};
use channels::{drain_side_channels, said};
...
///
/// **IT REPORTS THE CRASH AS WELL AS FREEING THE SLOT.** It used to only call
/// `lose`, which settles a PENDING turn and does nothing at all when there is
/// none — so a Worker that raised while idle (a throw between goals, a
/// listener that died) produced no status, no event and nothing on screen, and
/// the board went on rendering the row `idle`: "loaded and doing nothing;
/// nobody has called it" (`kernel::status`), for an agent that is dead. That
/// is I8 and I16 at once — a transition that emitted nothing, and a truth the
/// system held and did not state. `Status::Failed` is the variant whose own
/// definition is "it did not load, or its last turn raised", and it is written
/// on EVERY raise rather than only on the idle one: an agent whose turn just
/// died is failed whether or not anybody was waiting on it, and the next
/// `ready`/`Idle` it reports is what takes the row back.
/// (`tests/browser/tests/crashed_peer.rs`.)
fn on_error(
    who: String,
    pending: Rc<RefCell<Option<Turn>>>,
    queue: Rc<RefCell<Vec<(String, Status, String)>>>,
    ...
    Closure::wrap(Box::new(move |e: JsValue| {
        ...
        // QUEUED BEFORE THE TURN IS SETTLED, and the reason is the same one the
        // queue exists for: a rejection runs a caller's code, and this handler
        // is already inside a JS callback with the app borrowed.
        let row = format!("its Worker raised \"{raised}\"");
        queue.borrow_mut().push((who.clone(), Status::Failed, row));
...
pub(crate) fn listen(
    ...
    let raised = on_error(name.to_string(), Rc::clone(&waiting), queue);
