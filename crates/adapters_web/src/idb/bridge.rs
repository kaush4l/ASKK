//! The IDBRequest → Future bridge, and the one place a `JsValue` thrown by
//! IndexedDB becomes a typed `StoreError` (spike-idb plumbing). Every call in
//! this folder goes through these two functions and nothing else awaits an
//! `IdbRequest` by hand.
