...
/// §7's contract-test stage). Tier-1 logic WOULD run through the interpreter
/// here, and cannot: `crates/script` is deleted (increment 09, ADR-003), so
/// this body is unwritten for two reasons now, not one. Tier-0 built-ins run the
/// identical cases from `core`'s own tests — same runner, hosted natively (I3).
