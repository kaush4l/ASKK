...
//! (`proctable::rows`), so the pane and the model can never disagree about which
//! processes exist.
//!
...
//!
//! `procpanel.rs` owns the projection — which listing, which state, which
//! rows — so this file owns the module and its two routes and both hold the
//! 200-line rule (I12), the same split `files.rs` and `filelist.rs` have.
