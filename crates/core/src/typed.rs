//! WHAT A PERSON'S GESTURE RUNS. Split from `workspace.rs`, which owns the
//! capability gate and the single place a command actually runs, so both hold
...
pub(crate) async fn stop_command(app: &Rc<RefCell<App>>) {
    ...
    if let Err(e) = port.stop().await {
        let said = crate::workspace::unavailable(e);
        app.borrow_mut().append(EventKind::Custom {
            kind: crate::terminal::STOP_FAILED.into(),
