...
//! (R18-P0-1). The reader half of `agent::steer`, beside `failure/stopped_notice.rs` and
//! `failure/ending.rs` for the same reason: one payload, one wording, and the
...
use crate::chat::fold::{belongs_to, msg, NOTICE};
...
pub(crate) fn said(
    ...
    let list = match open {
        Some(false) => list.child(crate::chat::fold::abandoned()),
        _ => list,
    };
