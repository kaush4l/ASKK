...
//! behind that name — who wrote them and what their space granted.
//! `chat/transcript.rs` is the conversation; this one is the heading over it.
...
pub(crate) fn header(ctx: &Ctx, who: &str) -> String {
    ...
    let mine = ctx
        ...
        .map(|(_, by)| by.as_str());
    identity(spec, mine, crate::chat::memory_line::memory(ctx, who))
}
...
/// `agents::card_sentences::origin_line` renders in both places, so the two cannot disagree.
///
