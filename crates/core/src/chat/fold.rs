...
//! last line should say, and what the page has spent. `chat/transcript.rs` renders
//! the conversation; this file answers what the log knows about it.
...
pub(crate) fn msg(class: &str, speaker: &str, text: &str, files: &[String]) -> Fragment {
    ...
    row.child(crate::chat::markdown::said(text, files)).build()
}
...
/// rather than in `chat/transcript.rs` because the fold that decides it is here
...
pub(crate) fn belongs_to(kind: &EventKind, me: &str, who: &str) -> bool {
    ...
    match kind {
        ...
        EventKind::Custom { kind, payload_json } => match kind.as_str() {
            ...
            k if k == crate::chat::pane::TURN_STOPPED => match crate::chat::pane::stopped_agent(payload_json) {
                name if name.is_empty() => who == me,
                name => who == name,
            },
            "core.agent_error" => crate::failure::from_worker::agent_of(payload_json) == who,
            _ => false,
        },
        _ => false,
    }
