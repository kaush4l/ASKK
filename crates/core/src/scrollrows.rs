//! WHICH commands the workspace scrollback shows, and WHOSE they are. Split
//! from `scrollback.rs` (which owns how one row looks) for the same reason
//! `trace.rs` and `tracerow.rs` are two files: selecting the facts and
//! rendering them are two jobs, and both files hold the 200-line rule (I12).
//!
...
//! is exactly the rule `trace.rs` follows over the same two records.
//!
...
use crate::scrollback::{command_of, ran};
...
fn ours(ctx: &Ctx) -> Vec<Fragment> {
    ...
    for (i, kind) in ctx.recent.iter().enumerate() {
        ...
        if let EventKind::ToolInvoked { tool, args, ok, output } = kind {
            ...
            let by = match crate::asked::pop_typed(&mut typed, args) {
                Some(_) => "you",
                None => ctx.me.as_str(),
            };
    ...
    for (i, payload) in replayed {
        ...
        typed.remove(at);
        out.push((i, crate::scrollback::abandoned(&crate::asked::typed_command(payload), "you")));
    }
...
pub(crate) fn in_flight(ctx: &Ctx, typed: Option<&str>) -> Vec<Fragment> {
    ...
    for command in ctx.running.iter().map(String::as_str).chain(typed) {
        let age = ctx
            ...
            .and_then(|c| crate::inflight::age(ctx, c));
        out.push(crate::scrollback::echoed(command, "you", age));
    ...
    for call in ctx.calling.iter().filter(|c| c.tool == "exec") {
        ...
        let age = crate::inflight::age(ctx, call);
        out.push(crate::scrollback::echoed(&command, &ctx.me, age));
    }
    out
}
