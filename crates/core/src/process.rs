...
//! scripts answers in. The work itself is three siblings, so all four hold the
//! 200-line rule (I12): `procstart.rs` starts one, `proctable.rs` lists them
//! all, `procwatch.rs` reads and stops one. What is HERE is what all three
//! share and none of them owns.
//!
...
pub(crate) async fn run(
    ...
    use crate::procstart::{start_script, started};
    use crate::proctable as table;
    use crate::procwatch as watch;
    Some(match tool {
...
/// shape every script here and in `procwatch` answers in.
