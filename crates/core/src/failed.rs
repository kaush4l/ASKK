...
//! points at the view that has it (`pointer::Where`, R17-P1-3).

use kernel::EventKind;

use crate::{asked::Asked, dispatch::Ctx, pointer::Where};
...
pub(crate) fn is_failure(tool: &str, ok: bool, output: &str) -> bool {
    !ok && !crate::filelist::missing(tool, output)
}
...
fn turn_began(ctx: &Ctx, who: &str) -> Option<usize> {
    ctx.recent
        ...
        .enumerate()
        .filter(|(_, k)| {
            matches!(k, EventKind::UserMessage { .. }) && crate::fold::belongs_to(k, &ctx.me, who)
        })
        .map(|(nth, _)| nth)
        .next_back()
}
...
fn mine(ctx: &Ctx, who: &str, start: usize) -> (usize, usize, Where) {
    ...
    // (`trace.rs`), so the summary and the row cannot disagree about which
    // refusal was recovered (R16-P1-1).
    let mut retries = crate::vouch::Retries::default();
...
/// R16-P1-1 the conversation's own announcement reads it too (`calls::take`):
