...
//! never a second COUNT of it: `busy_names` lives here and `board/pane.rs` calls it
...
//! working, which is the defect class `board/row`'s `data-line` was written to
//! close.
//!
...
///
/// It was written inline in `board/pane.rs` and is here because two readers need
...
fn tile(name: &str, said: &str, status: &str) -> Fragment {
    FragmentBuilder::new("div")
        ...
        // tile fall through to the next tile's value (`agents/card`'s rule for
...
/// same number, and the same reading of it, that `board/row` prints per row
...
fn tokens(ctx: &Ctx) -> Fragment {
    let said = match crate::chat::fold::spent(ctx) {
...
/// agent. Here rather than in `row.rs` because a door is a thing a card ENDS
/// with, and both the card and this strip open the same two.
///
...
/// (`ui/board/mod.rs`, mirroring `ui/board/roster.rs::pressed`). A component per card
...
/// number.
///
/// `ui::shell::token_meter::grouped` is the same nine lines. They are not shared because
