...
//! Split from `repeat.rs` — which folds repeated FAILURES — so both hold the
//! 200-line rule (I12): "the same error twice" and "what this round of calls
//! did" are two folds over two different facts.
//!
...
pub(crate) struct Calls {
    ...
    /// how many failures a retry cleared — `vouch::Retries`, the same fold the
    /// trace and the board row run (R16-P1-1).
    doubted: usize,
    cleared: usize,
    retries: crate::vouch::Retries,
    ...
    hurt: crate::pointer::Where,
    all: crate::pointer::Where,
}
...
impl Calls {
    ...
    /// unqualified answer. `failed::is_failure` is the same predicate the trace
    ...
    /// `vouch::doubt` is the predicate the trace's own word comes from.
    pub(crate) fn note(&mut self, tool: &str, args: &str, ok: bool, output: &str) {
        ...
        let (bad, back) =
            (crate::failed::is_failure(tool, ok, output), self.retries.note(tool, args, ok));
        self.hurt.note(tool, bad || back);
        self.failed += usize::from(bad);
        self.cleared += usize::from(back);
        self.doubted += usize::from(crate::vouch::doubt(tool, args, ok, output).is_some());
    ...
    pub(crate) fn take(&mut self) -> Option<String> {
        ...
        // ONE COUNT AND ONE WORDING for what went wrong in here: `failed::note`
        // (R16-P1-1), which also owns the pointer (R17-P1-3).
        if let Some(clause) = crate::failed::note(failed, cleared, hurt) {
        ...
        Some(match (quiet, doubted) {
            ...
            (false, n) => format!("{what} — {}", crate::vouch::unbacked(n)),
        })
    }
}
