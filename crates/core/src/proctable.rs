//! THE LISTING — every process in one table, and the durations in it. Split
//! from `procwatch.rs`, which acts on ONE named process, so both hold the
//! 200-line rule (I12).
//!
...
/// The inverse of `ago`, over the three shapes it writes.
fn secs(word: &str) -> Option<i64> {
...
/// A duration a person and a model read the same way.
