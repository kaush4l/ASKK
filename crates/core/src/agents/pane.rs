...
//! them onto the running app is `agents/install.rs`; this file only renders.
...
pub(crate) fn agents(req: &Request, ctx: &mut Ctx) -> Response {
    match (req.method.as_str(), req.path.as_str()) {
        ...
        ("GET", "/agents/file") => crate::agents::authoring::file(req, ctx),
        ("POST", "/agents") => crate::agents::authoring::write(req, ctx),
        ("POST", "/agents/delete") => crate::agents::authoring::delete(req, ctx),
...
fn listing(ctx: &Ctx) -> Response {
    ...
    for spec in &ctx.agents {
        ...
        // make the call (`agents::card_sentences::model_line`). Absent when this build's port
        ...
        list = list.child(crate::agents::card::card(spec, &ctx.agents, &ctx.authored, found));
    }
