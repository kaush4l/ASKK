//! EVERY SENTENCE AN AGENT'S CARD PRINTS ABOUT IT — where it came from and what
//! it can therefore do, its real toolbox and the names that resolved to nothing
//! (R18-P1-7), who it can hand work to, the loop it runs and how many laps of it
//! a turn may take (31), and the model it calls. None of them is a route: they
//! are together because the agent card and the identity panel both draw them,
//! and each being ONE function is why those two cannot come to say different
//! things about one agent.
...
/// the critic by asking whether its file said `role: critic` — the wrong axis,
...
/// whether the agent had a FOLDER, and offered the critic ``Run `uname -a` in the
/// folder``. A folder is not permission to write in it. The RESOLVED
...
/// path check is legibility, not containment, stays in this comment and off the
/// card — R10-10.) `mine` is `Some(author)`: an empty author is the person at this keyboard, a
/// named one is the agent that wrote it. Both used to read "Authored in this
/// browser", so an agent a model wrote read as your own work (11b walk).
...
/// WHAT THIS AGENT'S NEXT TURN WOULD REALLY CALL. Every card on the Agents view
/// read `Model: local` — the file's `model:` field verbatim — at the moment the
/// header said "The next turn calls openrouter — openai/gpt-4o-mini" and
/// openrouter.ai was returning a real 401: somebody who changes the endpoint and
/// is refused reads the card and concludes the change did not take (cold walk,
/// 21). Two more errors — `model:` is a CATALOGUE KEY, not a model id
/// (`catalogue::Catalogue::resolve`), so `Model: local` labelled an endpoint as a
/// model; and an agent naming no key printed "Uses the endpoint's default model",
...
/// than "none": most agents delegate to nobody, and saying so on five of six
/// cards would bury the one where it matters.
...
/// THE LOOP THIS AGENT RUNS, in one line — `CHOOSES` above when it picks per
/// message. 20 shipped a declared loop no surface named (`verify`, `stage`,
/// `loop` and `delegat` each occurred zero times across all six views — cold
/// walk, 21); 31 added the lap count. `stages:` and `passes:` are the whole
/// source; this invents no state (I8).
