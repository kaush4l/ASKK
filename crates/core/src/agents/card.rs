//! ONE AGENT, as a card. `pane.rs` owns the route and the listing; this file
//! owns what one entry in that listing looks like.
//!
...
pub(crate) fn meta_line(
    ...
    let mut parts = vec![crate::agents::card_sentences::model_line(spec, found)];
    ...
    parts.extend(crate::agents::card_sentences::tool_lines(spec, peers));
    parts.join(" · ")
}
...
/// delegated handler (`ui/board/roster.rs`) turns a press into the route that already
...
fn doors(spec: &AgentSpec, peers: &[AgentSpec]) -> Fragment {
    ...
    if crate::agents::card_sentences::can(spec, peers) != "read" {
...
pub(crate) fn card(
    ...
    let mut card = FragmentBuilder::new("div")
        ...
        .attr("data-can", crate::agents::card_sentences::can(spec, peers))
    ...
    let mut runs = crate::agents::card_sentences::loop_line(spec);
    if let Some(peer) = crate::agents::card_sentences::peer_line(spec, peers) {
        runs.push_str(" · ");
        runs.push_str(&peer);
    }
    ...
    card.child(
            FragmentBuilder::new("p")
                .class("agent-origin")
                .text(&crate::agents::card_sentences::origin_line(spec, mine))
                .build(),
        )
