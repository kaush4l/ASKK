...
//! not. Its own file because the question is its own subject and the artifact
//! shelf asks it too, and because a wording about an agent's folder lives here
//! ONCE, so the panes that say it cannot drift apart.
...
/// this agent's file when it opens (`ui::authoring::agentfile::open_selected`), so the
...
/// OPENER and carries no trailing punctuation: `proc/rows` and `files/empty_states` build
...
/// pane names an agent's folder. One wording, one place: `space::pane::panel`
/// says it of the space path and `terminal/footnote.rs` of the same folder in the note
...
pub(crate) fn browsable(ctx: &Ctx, who: &str) -> Result<(), String> {
    ...
    if crate::terminal::row::space_of(ctx, who).is_none() {
    ...
    // once now (`ui::shell::rail`), so it is about the agent it is standing under, and
...
pub(crate) fn nothing_to_browse(why: &str) -> Response {
    html(
        ...
        module::view::FragmentBuilder::new("div")
            ...
            .child(crate::chat::markdown::said(why, &[]))
            .build()
            .into_html(),
    )
}
