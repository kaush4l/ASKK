...
//! `<name>\t<path>`. `listing.rs` owns the fold that finds the listing; this
//! file turns it into the lines the pane makes buttons out of.
//!
//! These rows never touch a wire — the seam is an in-process call — so a
//! tab-separated header is the whole format.

use crate::dispatch::Ctx;
use crate::files::listing::{newest_listing, parent};
...
pub(crate) fn rows(ctx: &Ctx, at: Option<&str>) -> String {
    let Some((at, output)) = newest_listing(ctx, at) else {
        return String::new();
    };
...
/// has no such folder, which fails, and `newest_listing` correctly answers "no listing"
