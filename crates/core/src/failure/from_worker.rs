//! A SUB-AGENT's failed turn, and the payload it travelled in. `agents/card.rs` is
//! what a failure looks like; this file is what a failure that happened
//! somewhere ELSE looks like once it has crossed a `postMessage` boundary.
...
/// (`failure::dedupe::Seen`): a sub-agent failing the same way five times wrote five
