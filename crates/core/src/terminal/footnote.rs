...
//! far the tools reaching it go. A row and a footnote about the machine the
//! rows ran on are two different things to write, so `row.rs` writes the first
//! and this file the second.

use module::view::{Fragment, FragmentBuilder};

use crate::dispatch::Ctx;
use crate::terminal::row::space_of;
...
/// The scanned line and the paragraph behind it. Its own fn because the two
/// halves are one decision and must be read as one.
fn workspace_said(
    theirs: &agent::Space,
    who: &str,
    me: &str,
    ...
    (
        ...
        format!(
            ...
            theirs.name,
            theirs.path(),
            crate::files::permitted::kept(durable)
        ),
    )
}
