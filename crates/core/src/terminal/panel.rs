...
//! it, and the assembly of the rows into one projection. `row_selection.rs`
//! decides WHICH rows and whose they are; this file is the container they land
//! in.
...
fn scroller(ctx: &Ctx, who: &str) -> FragmentBuilder {
    ...
    let root = crate::terminal::row::space_of(ctx, who)
        .map(|s| s.path())
...
fn nothing_yet(ctx: &Ctx, who: &str) -> module::view::Fragment {
    ...
    let alone = crate::terminal::row::space_of(ctx, who).is_none();
    let said = match (who == ctx.me, alone) {
        // …and this is the ONE place it is said on this view (R10-11), so it
        // carries the way out too — `files/permitted`'s wording, not a second one.
        (_, true) => format!(
            "{who} has no folder, so it runs no commands. {}",
            crate::files::permitted::GIVE_IT_A_SPACE
        ),
...
/// empty state — `nothing_yet` above — and this must not say it twice.
///
...
/// what a read-only agent IS. It asks `agents::card_sentences::can` — the one predicate the
...
fn can(ctx: &Ctx, who: &str) -> &'static str {
    ctx.agents
        ...
        .map_or("read", |spec| crate::agents::card_sentences::can(spec, &ctx.agents))
}
...
pub(crate) fn panel_with(ctx: &Ctx, who: &str, typed: Option<&str>) -> String {
    ...
    // `terminal/row_selection` owns it, and gives the tool trace the same answer.
    for row in crate::terminal::row_selection::commands(ctx, who) {
        list = list.child(row);
        count += 1;
    }
    ...
    if who == ctx.me {
        for row in crate::terminal::row_selection::in_flight(ctx, typed) {
            list = list.child(row);
            waiting += 1;
        }
    }
    ...
    format!(
        ...
        crate::terminal::footnote::note(ctx, who).map(module::view::Fragment::into_html).unwrap_or_default()
    )
}
