...
pub(crate) fn terminal(req: &Request, ctx: &mut Ctx) -> Response {
    ...
    match (req.method.as_str(), req.path.as_str()) {
        ("GET", "/terminal") => typeable(html(200, crate::terminal::panel::panel(ctx, &who)), ctx, &who),
...
fn typeable(mut response: Response, ctx: &Ctx, who: &str) -> Response {
    ...
    // on one (R16-P1-3).
    if let Some(why) = crate::terminal::panel::no_box_why(ctx, who) {
...
fn run(req: &Request, ctx: &mut Ctx, who: &str) -> Response {
    ...
    let mut response = html(200, crate::terminal::panel::panel_with(ctx, who, Some(&command)));
...
fn stop(ctx: &mut Ctx, who: &str) -> Response {
    ...
    typeable(html(200, crate::terminal::panel::panel(ctx, who)), ctx, who)
}
