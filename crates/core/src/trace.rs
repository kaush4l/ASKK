...
use crate::asked::{Asked, PANE};
use crate::reported::{at, reported};
...
pub(crate) fn trace(ctx: &Ctx, who: &str, app_activity: bool) -> Response {
    ...
    for (nth, kind) in ctx.recent.iter().enumerate() {
        ...
        if let EventKind::Custom { kind, payload_json } = kind {
            if kind == agent::STOPPED {
                list = list.child(crate::halted::row(who, payload_json, at(ctx, nth)));
                count += 1;
            }
        }
    ...
    for call in &ctx.calling {
        ...
        list = list.child(crate::inflight::running_row(call, crate::inflight::age(ctx, call), by));
        live += 1;
    }
    if count == 0 && live == 0 {
