...
fn newest(ctx: &Ctx, at: Option<&str>) -> (Option<Seen>, Option<Seen>) {
    ...
    for (i, kind) in ctx.recent.iter().enumerate() {
        ...
        // Same test `filegone` and `scrollrows` already apply to the same log.
...
/// printed. `filerows` turns it into the entries header; keeping the fold in
/// one place is what stops the two from disagreeing about which folder is on
/// screen.
pub(crate) fn listed(ctx: &Ctx, at: Option<&str>) -> Option<(String, String)> {
...
pub(crate) fn panel(ctx: &Ctx, at: Option<&str>) -> String {
    crate::filegone::folder(ctx, &newest(ctx, at).0).build().into_html()
}
...
/// because `x-at` is a FOLDER: `files.rs` scopes a file's request to the folder
