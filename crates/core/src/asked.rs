//! WHO ASKED for a tool call. Split from `trace.rs` — which owns which calls a
//! trace holds and how they are ordered — so both hold the 200-line rule (I12),
//! and because "who did this" is a question two readers ask: the trace and the
...
/// it: that index is the call's START (R13-4, `tracerow::when`).
pub(crate) fn pop_typed(typed: &mut Vec<(&str, usize)>, args: &str) -> Option<usize> {
    let ran = crate::scrollback::command_of(args);
...
/// text if it was never one. `pub(crate)` because `scrollrows` renders the
...
impl<'a> Asked<'a> {
    ...
    pub(crate) fn enqueue(&mut self, nth: usize, kind: &'a EventKind) {
        ...
        if kind == crate::files::OPEN_REQUEST || kind == crate::files::SAVE_REQUEST {
            if let Ok((path, _)) =
                serde_json::from_str::<(String, serde_json::Value)>(payload_json)
            {
                ...
                match kind == crate::files::SAVE_REQUEST {
                    true => {
    ...
    fn pop_path(&mut self, args: &str) -> Option<(&'static str, usize)> {
        let want = crate::filelist::path_of(args);
