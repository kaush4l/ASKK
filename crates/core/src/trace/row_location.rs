...
//! one reads. Its own file because it is a question about the TRACE, asked from
//! outside it — `failure/within_turn.rs` is the sentence that asks.
...
impl Where {
    ...
    pub(crate) fn note(&mut self, tool: &str, counted: bool) {
        ...
        match crate::trace::pane::is_shell(tool) {
...
mod tests {
    ...
    fn the_pointer_names_the_view_the_failing_call_is_actually_in() {
        ...
        assert!(crate::failure::within_turn::note(1, 0, shell).unwrap().ends_with("— Commands has it"));
        assert!(crate::failure::within_turn::note(1, 1, shell).unwrap().ends_with("— Commands has both"));
        assert!(crate::failure::within_turn::note(2, 0, both).unwrap().ends_with("— the Tool trace and Commands have them"));
        let mut tool = Where::default();
        tool.note("write_file", true);
        assert!(crate::failure::within_turn::note(1, 0, tool).unwrap().ends_with("— the Tool trace has it"));
    }
}
