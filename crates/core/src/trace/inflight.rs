...
/// no clock — the one case nothing may guess at (`board::row::live::elapsed`'s rule).
...
pub(crate) fn said(call: &Inflight) -> String {
    crate::trace::row::args::said_args(&call.tool, &call.args)
}
...
/// WHOSE CALL THE WORKSPACE IS INSIDE (R15-P0-1). `trace::requested_by::Asked` is the same
...
fn by_the_pane(ctx: &crate::dispatch::Ctx, call: &Inflight) -> bool {
    ...
    asked.actor(&call.tool, &call.args, &ctx.me).0 == crate::trace::requested_by::PANE
}
