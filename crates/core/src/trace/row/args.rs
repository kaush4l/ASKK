//! WHAT A CALL WAS ASKED TO DO, as a person reads it. Its parent owns the ROW —
//! its time, its outcome word, its output block. A row's shape and an
//! argument's rendering are two subjects, and `inflight` wants only this one.
...
pub(crate) fn said_args(tool: &str, args: &str) -> String {
    if tool == "exec" {
        return format!("$ {}", crate::terminal::row::command_of(args));
    }
