//! A SUB-AGENT'S trace, and the clock every row reads. Split from `trace.rs`,
//! which owns this page's own agent's calls, so both hold the 200-line rule
//! (I12) — and because they are records of two different kinds: `ToolInvoked`
//! facts this loop appended, against what another agent's Worker REPORTED.

use kernel::{EventKind, Response};
use module::view::FragmentBuilder;

use crate::dispatch::{html, Ctx};
use crate::tracerow::row;
...
/// A sub-agent's calls, as its own Worker REPORTED them. Its own fn so `trace`
/// holds the 40-line rule (I12), and because it is a different record: not
...
pub(crate) fn reported(ctx: &Ctx, who: &str) -> Response {
    ...
    for (nth, kind) in ctx.recent.iter().enumerate() {
        ...
        // Its shell is in Commands too (`scrollrows::reported`, R15-P1-4).
        if crate::trace::is_shell(tool) {
            shell += 1;
            continue;
        }
