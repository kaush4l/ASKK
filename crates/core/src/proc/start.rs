//! STARTING ONE, and the wrapper that makes it supervisable. `convention.rs`
//! owns the layout this writes into and the dispatch that calls it.
//!
...
//! come from the one script below. Every claim `proc/watch` and `proc/table`
//! make about a process is a claim about what this script left behind.

use kernel::{base64, shell_quote, Execution};

use crate::proc::convention::{blank_as, marked, word, DIR, STATE};
...
/// `failure::within_turn::is_failure` counts it: a summary carries the worst state it holds.
