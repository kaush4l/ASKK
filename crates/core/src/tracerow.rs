//! ONE ROW of the tool trace. Split from `trace.rs`, which owns which calls a
//! trace holds and in what order, so both hold the 200-line rule (I12).
//!
...
//! at`, injected via `ClockPort` — I7); the second is `traceargs`; the third
//! is the stylesheet's.
...
// The arguments' own rendering, which `inflight` also wants (I12).
pub(crate) mod traceargs;
...
/// log holds a START only where a REQUEST preceded the call (`asked::Asked`,
...
/// VOUCHING FOR WHAT IT DID NOT CHECK (R13-2): `crate::vouch` owns which
...
/// off `vouch::Retries`, fed the same calls in the same order.
pub(crate) fn row(
    tool: &str,
    args: &str,
    ok: bool,
    ...
    // …and only for a call ABOUT a path: `filelist::missing` holds why (R14-P0-3).
    let missing = !ok && crate::filelist::missing(tool, output);
    let word = match (ok, missing) {
        (true, _) => crate::vouch::word(crate::vouch::doubt(tool, args, ok, output)),
    ...
    // state an empty workspace has. `filegone` owns that sentence for both panes;
    // every other output is the model's own, verbatim.
    let said = match missing {
        true => crate::filegone::not_there(&crate::filelist::path_of(args)),
        false => result.line(),
    };
    FragmentBuilder::new("div")
        ...
        .child(
            FragmentBuilder::new("p")
                ...
                .child(
                    FragmentBuilder::new("span")
                        .text(&format!(" {}", traceargs::said_args(tool, args)))
                        .build(),
                )
                .child(
                    FragmentBuilder::new("span")
        ...
        .child(crate::vouch::folded(&said).unwrap_or_else(|| {
