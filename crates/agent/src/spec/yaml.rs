//! THE FRONTMATTER READER — every `key: value` an agent file may carry, one at
//! a time, and what each one refuses. The rules that need the whole file in
//! hand are the parent module's (`spec::refuse_contradictions`); these are the
//! rules one line can be judged by on its own.
//!
...
fn set_field(spec: &mut AgentSpec, key: &str, value: &str) -> Result<Option<&'static str>, AgentError> {
    match key {
        ...
        "engine" => spec.engine = one_of(spec, key, value, &[ENGINE_REACT, ENGINE_BASE])?,
...
fn number_field(spec: &mut AgentSpec, key: &str, value: &str) -> Result<(), AgentError> {
    match key {
