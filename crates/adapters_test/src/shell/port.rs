//! `FakeShell` AS A PORT: the `WorkspacePort` impl and the tests that hold it
//! to the same contract the real workspace answers. The parent owns the
//! fixture and its builders — what a shell is TOLD to say; this owns what it
//! then does when a caller runs a command.
