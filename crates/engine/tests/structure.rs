...
fn import_direction_is_one_way() {
    // (dir, forbidden substrings) — ADR-009/013 wire rules over the DAG
    // core ← inference ← state ← features ← engine ← browser ← frontend.
    // Each crate may only name crates below it; frontend imports core +
    // browser ONLY. A missing dir (planned surface) skips gracefully.
    let rules: &[(&str, &[&str])] = &[
        (
            "crates/core/src",
            &[
        ...
        (
            "crates/engine/src",
            &["askk_browser", "askk_frontend", "dioxus", "web_sys"],
        ),
        ("crates/browser/src", &["askk_frontend"]),
        (
            "crates/frontend/src",
            &[
                "askk_inference",
    ...
    assert!(
        violations.is_empty(),
        "import-direction violations (core ← inference ← state ← features ← engine ← browser ← frontend):\n  {}",
        violations.join("\n  ")
    );
}
...
fn every_agents_markdown_parses() {
    let root = workspace_root();
    let agents = root.join("crates/frontend/assets/agents");
