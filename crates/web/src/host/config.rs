...
// it finds (manifest.json fixes the order). `AGENT_FILES`, `SKILL_FILES`,
// `SOUL_MD`:
include!(concat!(env!("OUT_DIR"), "/agents_gen.rs"));

pub type ConfigSet = (Vec<AgentConfig>, Vec<SkillConfig>, String);
...
pub fn register_baked_tools(registry: &mut askk_runtime::tools::ToolRegistry) {
...
/// the font the app already ships.
#[cfg(target_arch = "wasm32")]
fn assets_base() -> String {
    use dioxus::prelude::*;
    const ANY: Asset = asset!("/assets/amarante-latin.woff2");
...
#[cfg(target_arch = "wasm32")]
pub async fn fetched_config(registry: &mut askk_runtime::tools::ToolRegistry) -> Option<ConfigSet> {
