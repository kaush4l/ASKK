...
//! view (VIEWS.md §5). Split from `stage.rs`, which routes the centre, so both
//! hold the 200-line rule (I12): what is IN the middle and what stands beside it
//! are two decisions, and only this one has state of its own.
...
use crate::{artifacts, files, listing, processes};
...
fn refused(web: &Signal<Option<Rc<WebApp>>>, agent: &str) -> Option<String> {
    ...
    let html = app
        .handle(Request::get("/files").with_header("x-agent", agent).with_header("x-at", "."))
        .body;
...
pub fn Rail(
    ...
    use_effect(move || {
        ...
        let now = refused(&web, &selected())
            .filter(|html| !crate::listing::spaceless(html))
            .unwrap_or_default();
        if *denial.peek() != now {
            denial.set(now);
        }
    ...
    rsx! {
        aside {
            ...
            if here == View::Workspace && !denial.read().is_empty() {
                ...
                processes::Processes { web, tick, agent: selected, at }
