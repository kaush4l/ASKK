//! The one control in Settings that DESTROYS something, and the confirmation it
//! asks for first. `view.rs` arranges the pane; a press that cannot be undone is
//! its own job, stated in one place so nothing else can grow one quietly.
