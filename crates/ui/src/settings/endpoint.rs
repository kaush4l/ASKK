...
//! gated on. What SETTINGS says about an address — the health line, the save
//! confirmation, the trust note, the base-URL rule — is `endpoint_copy.rs`
//! beside it. One file owns each half of "what this build says about an
...
pub(crate) fn endpoint_configured(web: Signal<Option<Rc<WebApp>>>) -> bool {
    ...
    !app.endpoint_summary().0.is_empty()
        || crate::settings::endpoint_copy::ondevice::is_on_device(&app, &app.current_entry())
}
...
pub(crate) fn endpoint_parts(web: Signal<Option<Rc<WebApp>>>) -> Parts {
    ...
    if crate::settings::endpoint_copy::ondevice::is_on_device(&app, &entry) {
        return (
...
/// the header's `Agent: {selected}` cluster. It has its own pill (`shell/status_pills.rs`).
///
