//! The Settings pane's MARKUP, and none of its decisions: every handler calls
//! `settings/mod.rs`, which calls the broker. What the pane LOOKS like and what
//! a press DOES are two jobs, and only the second may touch a credential.
...
use crate::settings::endpoint_copy::ondevice;
use crate::settings::endpoint_copy::{bad_base, unsaved};
...
fn key_row(web: Signal<Option<Rc<WebApp>>>, f: Fields, endpoint_set: Signal<bool>) -> Element {
    ...
    rsx! {
        ...
        div { class: "row",
            ...
            {crate::settings::endpoint_copy::reset::reset_control(web, f, endpoint_set)}
        }
    }
}

/// The form: pick an entry, override it, save. All markup — and the SPLIT's
/// reading column now (`settings/mod.rs`, R6-LAYOUT).
