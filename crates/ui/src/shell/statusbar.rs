//! The header's one strip of facts. Its own file because the strip has a RULE
//! of its own that is worth stating in one place; `main.rs` owns the shell that
//! mounts it and none of what it may say.
//!
...
use crate::shell::status_pills::Fleet;
use crate::ui::Disclosure;
use crate::shell::{status_pills, token_meter, warmth};
        ...
        Disclosure { summary: "What the status pills mean",
            ...
            p { class: "note",
                if spent == 0 {
                    ...
                    "Tokens, every agent: {token_meter::grouped(spent)}. Every token spent by every agent \
...
pub fn StatusStrip(
    ...
    /// The board's own poll, published to the chrome (`status_pills::Fleet`).
    fleet: Fleet,
    tokens: Signal<u64>,
    ...
    endpoint: crate::settings::endpoint::Parts,
) -> Element {
    rsx! {
        ...
        crate::shell::views::misroute::Misroute { hushed: !fleet.why.read().is_empty() }
        ...
        div { class: "status-strip", tabindex: "0", role: "group",
            ...
            if ready {
                ...
                // own row under this one (`main.rs`, `status_pills::TroublePill`).
                status_pills::RunPill { fleet }
                warmth::WorkspaceWarmth { who: selected, agents }
                token_meter::TokenMeter { tokens }
                // LAST, and the only one that gives way — by SHRINKING, never
                // by leaving (R5-7, R11-10).
                crate::settings::endpoint::EndpointPill { parts: endpoint }
            }
        }
    }
}
