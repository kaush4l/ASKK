...
//! `procrows.rs` owns one row — what it says and what pressing it does — so both
//! files hold the 200-line rule (I12).
...
use crate::listing::{TICK_MS, WATCH_TICKS};
use crate::procrows::{procrow, read, Proc};
...
pub fn Processes(
    ...
    // second clock (the rule `files.rs` and `artifacts.rs` follow).
