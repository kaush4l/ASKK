...
use crate::endpointform::{saved_line, EndpointHealth, TrustNote};
use crate::settings_view::endpoint_form;
use crate::ui::Card;
    ...
    spawn(async move {
        match app.set_endpoint(&entry, &url, key.as_deref(), &model).await {
            Ok(()) => {
                ...
                crate::trouble::hush_current(&app);
...
/// report of what was. A refused Base URL is not one: it goes under its field (R4-7).
fn refusal(status: &str) -> bool {
    ["cannot call", "not saved yet"]
        .iter()
        .any(|mark| status.contains(mark))
}
