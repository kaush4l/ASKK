...
//! writes when nothing has changed for a while. `state.rs` beside it owns what
//! the pane is SHOWING; this owns the waiting, which is the half with a clock.
...
pub(crate) async fn watch(
    ...
    for tick in 1.. {
        ...
        if silent >= STALL_TICKS {
            turn.note.set(crate::chat::inflight_row::stalled(silent * TICK_MS as u32 / 1000, &web, &who));
        }
    }
}
