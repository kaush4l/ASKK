...
//! turn to its end. `chat/mod.rs` owns the pane; this owns what the pane is
//! SHOWING, as one value the heading and the transcript both read.
...
pub(crate) fn show(asked: &str, res: Response, mut turn: Turn) {
    ...
    spawn(async move {
        ...
        crate::shell::route::newest_turn(&scrolled);
    });
}
