...
//! `state.rs` next door owns the turn itself; this owns the waiting, and the
//! meter rides the same poll.
...
/// What `core::trace::inflight::doing` says when the model is the thing outstanding.
...
fn stop_waiting(web: Signal<Option<Rc<WebApp>>>, turn: Turn, who: &str) {
    ...
    // with `core::chat::fold::STOPPED`, which said the same thing in nearly the same
...
pub(crate) fn stalled(silent: u32, web: &Signal<Option<Rc<WebApp>>>, who: &str) -> String {
    let (board, doing) = crate::board::read_attrs::progress(web, who);
