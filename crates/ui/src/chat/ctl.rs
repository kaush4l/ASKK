...
//! that ends it. Split out of `chat.rs`, which was at exactly 200 lines (I12)
//! and owns the conversation itself.
...
/// with no `PartialEq`, which is what Dioxus's props derive needs. `chat.rs`
/// passes the same one to `waiting_row` for the same reason.
