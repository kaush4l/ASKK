...
//! `proc/mod.rs` beside it owns the pane, its poll and the list; this owns what
//! ONE row says and the two presses on it. What is running and what one process
//! is are two jobs, and only the first has a clock.
//!
...
use crate::files::listing::open_path;
use crate::ui::Button;
...
/// whole of it on the row's `title` (R10-1).
