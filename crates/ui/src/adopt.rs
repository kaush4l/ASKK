...
//! the boot plumbing `main.rs` has no room for. Split from `dash.rs`, which
//! owns the panel switches and the viewport listeners, so both hold the
//! 200-line rule (I12).
...
pub fn watch_agents(
    ...
    if !names.is_empty() && !names.iter().any(|n| n == &*selected.peek()) {
        let fallback = names.iter().find(|n| *n == crate::route::DEFAULT_AGENT);
