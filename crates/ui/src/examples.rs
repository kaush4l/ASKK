...
//!
//! The ROW is here too: the sets and their buttons are one job (I12).
...
fn facts(board: &str, who: &str) -> (Option<String>, u16) {
    let laps = crate::runstatus::cell(board, who, "data-laps").and_then(|n| n.parse().ok());
    (crate::runstatus::cell(board, who, "data-toolset"), laps.unwrap_or(1))
}
