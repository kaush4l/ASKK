...
//! rather than a record — the one control that writes it back. `files/mod.rs`
//! owns the folder and its watcher: a folder and the one file open in it are
//! two subjects, and only this one can be typed into.
//!
...
/// folder is `core::proc::convention::DIR`'s shape and this is the one place the UI has
/// to know it — the same string `row.rs` builds a log's path from.
