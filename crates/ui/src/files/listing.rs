...
//!
//! `files/mod.rs` watches the workspace root and `files/artifacts.rs` watches
...
//! copies of the same twenty lines. The parse has one home here: the panes own
//! their arrangement, this owns what a listing IS.
//!
...
/// OPENING one entry, as a request: a `POST /files` naming the path and whether
/// it is a folder. The one builder for both shapes — the Files pane consumes the
/// response as its new listing and everyone else fires and forgets, and that
/// difference is in what the caller does with it, never in what it sends.
pub(crate) fn opening(path: &str, folder: bool) -> Request {
...
pub(crate) fn open_path(web: Signal<Option<Rc<WebApp>>>, agent: &str, path: &str, folder: bool) {
    ...
    app.handle(opening(path, folder).with_header("x-agent", agent));
}
