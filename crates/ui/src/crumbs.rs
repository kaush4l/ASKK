//! WHICH FOLDER THE FILES PANE IS ON (R16-P1-4). Split from `files.rs`, which
//! owns the folder, its watcher and the editor, so both hold the 200-line rule
//! (I12): where you are and what is there are two things to say.
//!
...
/// cannot navigate is decoration. The root's name is `filegone::named`'s word
...
/// system already has for "where you are" (`views::DesignCrumb`), and
