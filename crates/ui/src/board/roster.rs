...
//! agent — `authoring/mod.rs` owns the form, this owns the list.
...
/// three guesses disagreed on the same screen. `board::read_attrs::cell` is the same
/// one-bit read of a rendered attribute the board rows already get.
pub(crate) fn space_of(agents: &str, who: &str) -> Option<String> {
    crate::board::read_attrs::cell(agents, who, "data-space")
}
...
/// belongs to the space (`core::terminal::footnote::workspace_said`).
