//! READING THE BOARD PROJECTION — every read of `/board` that is not a
//! sentence: the one-bit reads of a rendered attribute that `ui::has_rows` and
//! `terminal::commands_in` already make, and the failure headers the response
//! carries beside them. `launch/outcome.rs` next door owns what the launch card
//! SAYS about a run; this owns how anything finds out.
...
/// One header off a response by name, empty when it is not there. Absent and
/// empty are the same fact for every header read here: the core writes these
/// only when it has something to say.
pub(crate) fn header(res: &Response, name: &str) -> String {
    res.headers.iter().find(|(k, _)| k == name).map(|(_, v)| v.clone()).unwrap_or_default()
}

/// THE FAILURE THE `/board` RESPONSE IS CARRYING — the three headers that
/// travel together. `x-failed` alone is only half a fact: two turns can fail
/// with the same sentence, and the turn count is what tells them apart, so
/// whoever reads one of these reads all three.
pub(crate) struct Failure {
    /// The one-line reason (`x-failed`). Empty means nothing has failed.
    pub(crate) why: String,
    /// Whose turn it was (`x-failed-agent`). Empty when the core did not name one.
    pub(crate) who: String,
    /// WHICH failure (`x-failed-turn`) — the agent's turn count at the time.
    pub(crate) turn: String,
}

/// The three read together, off a projection already in hand.
pub(crate) fn failure(res: &Response) -> Failure {
    Failure {
        why: header(res, "x-failed"),
        who: header(res, "x-failed-agent"),
        turn: header(res, "x-failed-turn"),
    }
}

/// WHICH AGENT A PRESS INSIDE THE BOARD WAS ABOUT (27, R15-P1-9), and where it
/// goes: `(data-open, data-agent)`.
///
/// A CARD IS A DOOR. The board and the agent deck are both the core's own
/// markup, so there is no component to hang an `onclick` on — the container
/// carries `data-agent`, the button inside it carries `data-open`, and one
/// delegated handler per pane turns a press into a route that exists. The two
/// panes differ by their container's class and by nothing else, which is why
/// that is the argument and this is one function rather than two copies.
pub(crate) fn pressed(event: &Event<MouseData>, container: &str) -> Option<(String, String)> {
    use wasm_bindgen::JsCast;
    let target = event.downcast::<web_sys::MouseEvent>()?.target()?;
    let button = target.dyn_ref::<web_sys::Element>()?.closest("[data-open]").ok()??;
    let card = button.closest(container).ok()??;
