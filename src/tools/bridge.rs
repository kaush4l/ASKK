...
/// Build the bridge MCP relay endpoint for `op` (`spawn` / `send` / `kill`) from the
/// configured tools base URL. The MCP routes live next to the tools route on the same
/// bridge origin (`/askk/mcp/<op>` vs `/askk/tools/<tool>`), so we derive the origin
/// from [`WebSearchToolConfig::bridge_tools_url`] (default
/// `http://127.0.0.1:8874/askk/tools`) by stripping the trailing `/askk/tools`. This
/// keeps the process-backed MCP transport pointed at the same bridge the user already
/// configured for tools.
// Consumed only by the wasm-only `BridgeMcpTransport`; on the host build nothing in
// the binary calls it (the unit test does), so it reads as dead code there.
#[cfg_attr(not(target_arch = "wasm32"), allow(dead_code))]
pub(crate) fn bridge_mcp_endpoint(config: &WebSearchToolConfig, op: &str) -> AppResult<String> {
    let trimmed = config.bridge_tools_url.trim().trim_end_matches('/');
    if trimmed.is_empty() {
        return Err("ASKK bridge tools URL is empty. Set it on the Tools page.".to_string());
    }
    if !(trimmed.starts_with("http://") || trimmed.starts_with("https://")) {
        return Err(format!(
            "ASKK bridge tools URL must start with http:// or https://: {trimmed}"
        ));
    }
    // Derive the bridge origin: drop a trailing `/askk/tools` path segment (and any
    // trailing slash left behind) so `.../askk/tools` -> `...` -> `.../askk/mcp/<op>`.
    let origin = trimmed
        .strip_suffix("/askk/tools")
        .unwrap_or(trimmed)
        .trim_end_matches('/');
    Ok(format!("{origin}/askk/mcp/{op}"))
}
...
mod tests {
    ...
    #[test]
    fn bridge_mcp_endpoint_derives_origin_from_tools_url() {
        let config = WebSearchToolConfig::default();
        assert_eq!(
            bridge_mcp_endpoint(&config, "spawn").unwrap(),
            "http://127.0.0.1:8874/askk/mcp/spawn"
        );
        assert_eq!(
            bridge_mcp_endpoint(&config, "send").unwrap(),
            "http://127.0.0.1:8874/askk/mcp/send"
        );
        assert_eq!(
            bridge_mcp_endpoint(&config, "kill").unwrap(),
            "http://127.0.0.1:8874/askk/mcp/kill"
        );

        // A trailing slash on the configured tools URL is tolerated.
        let trailing = WebSearchToolConfig {
            bridge_tools_url: "http://127.0.0.1:8874/askk/tools/".to_string(),
            ..WebSearchToolConfig::default()
        };
        assert_eq!(
            bridge_mcp_endpoint(&trailing, "spawn").unwrap(),
            "http://127.0.0.1:8874/askk/mcp/spawn"
        );

        // A base that is not the canonical `/askk/tools` path is used as the origin
        // verbatim (just the op appended under `/askk/mcp`).
        let custom = WebSearchToolConfig {
            bridge_tools_url: "https://bridge.example.com".to_string(),
            ..WebSearchToolConfig::default()
        };
        assert_eq!(
            bridge_mcp_endpoint(&custom, "send").unwrap(),
            "https://bridge.example.com/askk/mcp/send"
        );
    }

    #[test]
    fn bridge_mcp_endpoint_rejects_non_http_base() {
        let bad_scheme = WebSearchToolConfig {
            bridge_tools_url: "ftp://localhost/askk/tools".to_string(),
            ..WebSearchToolConfig::default()
        };
        assert!(bridge_mcp_endpoint(&bad_scheme, "spawn").is_err());

        let empty = WebSearchToolConfig {
            bridge_tools_url: String::new(),
            ..WebSearchToolConfig::default()
        };
        assert!(bridge_mcp_endpoint(&empty, "spawn").is_err());
    }
