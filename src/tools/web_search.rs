...
/// all of them land in the model's context together. The reader now strips page chrome
/// and its metadata preamble before this cap, so the budget holds real article text
/// rather than nav menus — worth a little more room per page than the old 4k.
const MAX_SCRAPE_CHARS: usize = 6_000;
