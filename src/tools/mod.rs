...
impl ToolRegistry {
    ...
    pub async fn execute(
            ...
            .find(|descriptor| descriptor.spec.name == tool_name)
        {
            ...
            // No descriptor registered under this name: a typed [`EngineError::Tool`],
            // lowered to the `AppResult` `String` alias at this boundary so the
            // `ToolResult` content (and every existing caller) is unchanged in shape.
            None => Err(EngineError::Tool(format!("Unknown compiled tool: {tool_name}")).into()),
        };
...
fn register_builtin_tools(registry: &mut ToolRegistry) {
    ...
    registry.register(workspace::open_descriptor());
    registry.register(workspace::close_descriptor());
    registry.register(read_run_output::descriptor());
    registry.register(call_agent::descriptor());
    registry.register(delegate_team::descriptor());
    registry.register(agent_mailbox::send_descriptor());
    registry.register(agent_mailbox::progress_descriptor());
    registry.register(agent_mailbox::list_descriptor());
