...
//! workspace filesystem (OPFS, rooted at the `workspace` directory; see
//! [`crate::storage::opfs_vfs`]). Complements `file_write` (whole-file overwrite):
//! the agent edits a file the way a person does in the Workspace editor — change one
//! spot, keep the rest — without re-sending the whole document.
//!
//! It targets the SAME OPFS as `file_read`/`file_write`/`file_list` and the workspace
//! IDE, so an edit lands where the user (and the `## WORKSPACE` view) sees it.
//! (Previously it wrote to the legacy IndexedDB `ProjectVfs`, which is now only the
//! one-time migration source — its writes never reached the live filesystem.)
//!
//! The replacement core is a pure function ([`apply_replacement`]) so the
//! ambiguity rules are host-testable without OPFS.
//!
