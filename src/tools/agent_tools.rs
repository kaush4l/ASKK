...
//! per-agent sugar over the same engine-spawn seam: every enabled peer agent is
//! offered as its own tool named `agent_<slug>` (e.g. `agent_researcher`). This
//! module owns only the NAMING and allowlist side — the stable `agent_<slug>` ->
//! agent-id assignment ([`assignments`]/[`resolve`]), the names a run may offer
//! ([`candidate_names`]), and the advertised [`ToolSpec`]s ([`specs_for_agent`]).
//! The shell turns each name into an [`EngineTool`](super::engine_tool::EngineTool)
//! — the engine-as-a-tool — when it assembles the run's tool set, so the loop
//! dispatches a specialist polymorphically with no name special-casing.
//!
...
//! instruction (CLAUDE.md invariant 3).
