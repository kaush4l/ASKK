...
thread_local! {
    /// The live process table and its change-counter, behind the shared
    /// [`VersionedVec`] helper: every mutation that alters the entry set bumps a
    /// monotonic version the UI polls (see [`version`]). Insertion order is
    /// preserved, which [`list`] relies on.
    static PROCESSES: RefCell<VersionedVec<Entry>> = const { RefCell::new(VersionedVec::new()) };
...
pub fn register(
    ...
    // `push` bumps the store's version, so the UI poller sees the new process.
...
pub fn kill(id: u64) {
    // `remove_first` removes the matching entry and bumps the store's version
    // (only on a real removal). The teardown closure runs *after* the borrow is
    // released, so a re-entrant teardown cannot deadlock the store.
    let on_kill = PROCESSES.with(|processes| {
        processes
            .borrow_mut()
            .remove_first(|entry| entry.info.id == id)
            .and_then(|mut entry| entry.on_kill.take())
    });
...
pub fn unregister(id: u64) {
    // `retain` bumps the store's version only when an entry is actually dropped,
    // so completion after a kill (the id already gone) is a no-op for pollers.
    PROCESSES.with(|processes| {
        processes.borrow_mut().retain(|entry| entry.info.id != id);
    });
}
...
pub fn version() -> u64 {
    PROCESSES.with(|processes| processes.borrow().version())
}
