...
use crate::state::{
    ...
    RunId, RunLane, RunScratchpad, RunStatus, ScratchpadObservation, ToolSpec, default_tool_names,
    event, now_iso, rolling_summary_for, upsert_rolling_summary,
};
use crate::strategy::{
    DeclaredStrategy, LoopMode, MAX_BACK_EDGES, Phase, PhaseOutcome, Routing, StrategyContext,
    StrategyHandle, StrategyRegistry, ToolPolicy, fallback_strategy, resolve_strategy_id,
    static_handle,
};
...
/// The owned Signal SINK a caller hands to the run (DUAL-EMIT). The run reborrows
/// it as the hook layer's [`session::SignalSink`] and also drives it directly from
/// the strategy driver. Only the worker boundary supplies a `Some`; every other
/// caller passes `None`, leaving the legacy observer path the sole live stream and
/// its behavior unchanged. The boundary that owns the `Some` (the worker runtime)
/// assigns `seq`/`ts_ms`/`run_id` and posts the full `Signal`; this layer stays
/// pure and supplies only `(instance, kind)`.
pub type OwnedSignalSink = Option<Box<dyn FnMut(InstanceName, SignalKind)>>;
...
thread_local! {
    /// Cooperative per-instance stop set, keyed by [`RunId`]. WASM is
    /// single-threaded, so a thread-local set is sufficient to signal an in-flight
    /// run to halt after its current turn. Replaces the old single global flag so
    /// the fleet can interrupt one instance by id without halting the others.
    static INTERRUPTS: RefCell<HashSet<RunId>> = RefCell::new(HashSet::new());
    /// Cooperative per-instance pause set, keyed by [`RunId`]. Scaffolding for the
    /// fleet's pause/resume control; the loop does not consult it yet (a pause is
    /// driven through the workflow gate / provider-exhaustion paths today), but
    /// the keyed knob exists so control is per-instance from the start.
    static PAUSES: RefCell<HashSet<RunId>> = RefCell::new(HashSet::new());
}

/// Request that the run with `id` stop after the current turn.
pub fn request_interrupt(id: &RunId) {
    INTERRUPTS.with(|set| {
        set.borrow_mut().insert(id.clone());
    });
}

/// Clear any pending interrupt for `id` before starting (or resuming) its run.
pub fn clear_interrupt(id: &RunId) {
    INTERRUPTS.with(|set| {
        set.borrow_mut().remove(id);
    });
}

/// Whether an interrupt is pending for the run with `id`.
pub fn interrupt_requested(id: &RunId) -> bool {
    INTERRUPTS.with(|set| set.borrow().contains(id))
}

/// Mark the run with `id` as paused (per-instance control scaffolding).
///
/// Pause/resume are the keyed control surface the fleet drives by id; the loop's
/// pause today still flows through the workflow-gate / provider-exhaustion paths,
/// so these are wired-but-unconsulted until the fleet control unit lands.
#[allow(dead_code)]
pub fn pause(id: &RunId) {
    PAUSES.with(|set| {
        set.borrow_mut().insert(id.clone());
    });
}

/// Clear the paused flag for the run with `id` (resume scaffolding).
#[allow(dead_code)]
pub fn resume(id: &RunId) {
    PAUSES.with(|set| {
        set.borrow_mut().remove(id);
    });
}

/// Whether the run with `id` is currently flagged paused.
#[allow(dead_code)]
pub fn pause_requested(id: &RunId) -> bool {
    PAUSES.with(|set| set.borrow().contains(id))
}
...
impl SessionRunner {
    ...
    {
        // No Signal sink: the legacy observer is the only live stream (unchanged).
        self.run_with_params_observer_and_signal(snapshot, goal, params, observer, None)
    }

    /// Run a goal as [`Self::run_with_params_and_observer`], plus a parallel
    /// Signal SINK (DUAL-EMIT). The observer path is identical and unchanged; the
    /// sink, when `Some`, additionally receives every `(instance, kind)` delta the
    /// run emits. The worker runtime is the sole caller that passes `Some` — it
    /// turns each delta into a `WorkerEvent::Signal`.
    pub fn run_with_params_observer_and_signal<F>(
        &self,
        snapshot: AppSnapshot,
        goal: String,
        params: LoopParams,
        observer: F,
        signal: OwnedSignalSink,
    ) -> BoxFuture<'_, AppSnapshot>
    where
        F: FnMut(AgentRun) + 'static,
    {
        ...
        Box::pin(async move {
            run_react_session(executor, snapshot, goal, params, observer, signal).await
        })
    }
        ...
        F: FnMut(AgentRun) + 'static,
    {
        ...
        Box::pin(async move {
            ...
            match goal {
                Some(goal) => {
                    // Resume carries no Signal sink today; the observer path is
                    // the sole live stream, unchanged.
                    run_react_session(
                        executor,
                        snapshot,
                        goal,
...
struct RunSession {
    ...
    lane: RunLane,
    /// The strategy driving this run's phase sequence. An `Rc` so a built-in
    /// (`&'static`) and an `agent.md`-declared (heap-owned) strategy share one field
    /// and one set of call sites.
...
impl RunSession {
    ...
    fn new(
        ...
        // Resolve the provider CONNECTION (per-agent profile first, then the workspace
        // active profile, then the global provider). This is what lets each agent in a
        // team target a different LLM endpoint/key/model and run in parallel — a profile
        // is matched by id or (case-insensitive) name so `.md` frontmatter can name it.
        let provider_profile_id = agent
            .provider_profile_id
            .clone()
            .or_else(|| snapshot.active_provider_profile_id.clone());
        let mut provider = provider_profile_id
            .and_then(|wanted| {
                snapshot.provider_profiles.iter().find(|profile| {
                    profile.id == wanted || profile.name.eq_ignore_ascii_case(&wanted)
                })
            })
            .map(|profile| profile.config.clone())
            .unwrap_or_else(|| snapshot.provider.clone());

        // Then resolve the model profile (per-agent first, then the workspace active
        // profile) and apply its tuning onto the chosen provider config for this run.
        ...
        // Strategy resolution precedence: an explicit `LoopParams.strategy` override
        // beats the agent's declared phases, which beat the agent's `strategy_id`,
        // which beats the default `react`. A declared-phase agent (`phases: Some`) with
        // no explicit override builds a runtime `DeclaredStrategy` instead of a registry
        // lookup. When a configured id is unknown, fall back to `react` and remember the
        // id so `run` can surface a run event (the run does not exist yet here).
        let registry = StrategyRegistry::new();
        let explicit_param = params
            .strategy
            .as_deref()
            .map(str::trim)
            .filter(|value| !value.is_empty());
        let (strategy, unresolved_strategy): (StrategyHandle, Option<String>) = match explicit_param
        {
            // Explicit override wins over everything, declared phases included.
            Some(_) => {
                let strategy_id =
                    resolve_strategy_id(params.strategy.as_deref(), agent.strategy_id.as_deref());
                match registry.get(&strategy_id) {
                    Some(strategy) => (static_handle(strategy), None),
                    None => (static_handle(fallback_strategy()), Some(strategy_id)),
                }
            }
            // No override: declared phases (if any) drive the run directly.
            None => match agent.phases.as_deref().filter(|phases| !phases.is_empty()) {
                Some(declared) => (
                    Rc::new(DeclaredStrategy::from_declared(agent.id.clone(), declared)),
                    None,
                ),
                None => {
                    let strategy_id = resolve_strategy_id(None, agent.strategy_id.as_deref());
                    match registry.get(&strategy_id) {
                        Some(strategy) => (static_handle(strategy), None),
                        None => (static_handle(fallback_strategy()), Some(strategy_id)),
                    }
        ...
        F: FnMut(AgentRun),
    {
        ...
        engine.base.phase_context = phase.context_line();

        let goal_text = phase_goal(phase, context, &run.goal, &self.skill_library);
        let artifacts_emitted = run.scratchpad.artifacts.len();
        // Mirror the live run into the snapshot the engine dispatches against so a
        // tool that writes to `current_run` (e.g. an image capture's
        // `attach_image_artifact`) has a populated target on its snapshot clone —
        // `run` stays the authoritative store, and `dispatch_tools` lifts each
        // call's new artifacts back onto it via `on_artifacts_appended`.
        snapshot.set_current_run(Some(run.clone()));
        let mut hooks = session::RunHooks {
            agent_loop: self,
            run,
            observer,
            signal: signal.as_deref_mut(),
    ...
    where
        F: FnMut(AgentRun),
    {
        if interrupt_requested(&RunId::from(run.id.clone())) {
            mark_interrupted(run, "Run interrupted before the next model call.");
            observer(run.clone());
            return None;
        }
    ...
    fn build_request(
        ...
        InferenceRequest {
            ...
            phase_context: phase.context_line(),
            // Internal side-calls (compaction, rolling-summary merge) never carry the
            // workspace block; only the live per-turn render does.
    ...
    async fn run<F>(
        ...
        goal: String,
        mut observer: F,
        mut signal: OwnedSignalSink,
    ) -> AppSnapshot
    where
        F: FnMut(AgentRun),
    {
        // The emitting component's bus address for driver-level signals: the
        // engine's BaseEngine name (the agent's display name). Captured up front
        // so the direct `emit` calls below stay terse.
        let instance = InstanceName(self.agent.name.clone());
        // DUAL-EMIT helper for the strategy driver's own emit points (the hook
        // layer has its own). A no-op when no sink is wired. Pure: it forwards
        // only `(instance, kind)`; the worker boundary stamps seq/ts/run_id.
        macro_rules! emit_signal {
            ($kind:expr) => {
                if let Some(sink) = signal.as_deref_mut() {
                    sink(instance.clone(), $kind);
                }
            };
        }
        ...
        observer(run.clone());
        // DUAL-EMIT: seed the reducer with the run's immutable identity FIRST,
        // mirroring the `AgentEventKind::Started` event `build_run` just pushed.
        emit_signal!(SignalKind::RunStarted {
            id: run.id.clone(),
            goal: run.goal.clone(),
            lane: run.lane,
            created_at: run.created_at.clone(),
        });
        // DUAL-EMIT: mirror the universal routing row `build_run` just pushed
        // (lane + compiled tools) onto the bus, folded into the generic phase
        // channel exactly as `SignalKind::from_legacy` maps `Routing`. Without
        // this the bus event log would miss every run's first informative row
        // until the terminal reconcile.
        emit_signal!(SignalKind::Phase {
            name: routing_title(run.lane),
            done: false,
        });
        ...
        // Sole-exit gate: only a `Done` from this phase ends the run as a success.
        // A gateless strategy (`react`, `orchestrate`) is satisfied by default, so its
        // terminal-phase `Done`/fall-off completes exactly as before.
        let gate_phase = self.strategy.gate_phase();
        let has_gate = gate_phase.is_some();
        let mut gate_satisfied = !has_gate;
        ...
        while phase_idx < phases.len() {
            ...
            // DUAL-EMIT: the phase entered (done:false), mirroring PhaseStarted.
            emit_signal!(SignalKind::Phase {
            ...
            if !apply_phase_gate(
                ...
                observer(run.clone());
                // DUAL-EMIT: the gate blocked the transition and paused the run
                // (apply_phase_gate set RunStatus::Paused).
                emit_signal!(SignalKind::StatusSet {
            ...
            let Some(outcome) = outcome else {
                ...
                // DUAL-EMIT: surface the terminal status the phase runner just set
                // (Interrupted/Paused/Error), so the bus reflects the stop. The
                // hook layer already emitted the matching kind for the model-failure
                // and answer-abort paths; this covers the interrupt edge that the
                // phase runner writes directly via `mark_interrupted`.
                if run.status == RunStatus::Interrupted {
                    emit_signal!(SignalKind::Interrupted);
                }
                break;
            };
            ...
            let raw_routing = self.strategy.route(phase_idx, &outcome);
            // `apply_back_edge_budget` turns a `Back` past the budget into `Done`;
            // remember that so the gate ends `Unverified` instead of a false success.
            let capped = matches!(raw_routing, Routing::Back(_))
                && context.back_edges_used >= MAX_BACK_EDGES;
            let routing = apply_back_edge_budget(raw_routing, &mut context.back_edges_used);
            // A real success: the designated gate phase emitted `Done` directly — not a
            // budget-capped Back, not a non-gate phase. Gateless strategies always count.
            let gate_done = matches!(routing, Routing::Done)
                && !capped
                && (gate_phase == Some(phase_idx) || !has_gate);
                ...
                && gate_done
            {
                ...
                match try_finalize_answer(
                    ...
                    None => {
                        // DUAL-EMIT: the OneShot-ReAct phase produced the validated
                        // final answer directly (this path bypasses the on_answer
                        // hook, so emit the same Result + passing-Verification pair).
                        last_answer = Some(final_text);
                        emit_signal!(SignalKind::Result {
                            final_text: run.final_answer.clone(),
                        });
                        emit_signal!(SignalKind::Verification { passed: true });
                    }
                    Some(feedback) => {
                        ...
                        emit_signal!(SignalKind::Verification { passed: false });
                    }
                }
                observer(run.clone());
            }
            ...
            observer(run.clone());
            // DUAL-EMIT: the phase finished (done:true), mirroring PhaseCompleted.
            emit_signal!(SignalKind::Phase {
                name: phase.name.to_string(),
                done: true,
            });
            ...
            match routing {
                ...
                // The gate (or a gateless strategy's terminal phase) approved: success.
                Routing::Done if gate_done => {
                    gate_satisfied = true;
                    break;
                }
                // The gate exhausted its revise budget without passing: stop now and let
                // finalize stamp the run `Unverified` (gate_satisfied stays false).
                Routing::Done if capped => break,
                // A non-gate phase cannot end the run — only the gate can. Advance; if this
                // was the last phase, falling off the loop end stays `Unverified`.
                Routing::Done => phase_idx += 1,
            }
        }
        ...
        finalize_status(&mut run, answered, gate_satisfied);
        // DUAL-EMIT: the run's terminal status is now fixed (finalize_status
        // flips Running → Complete). Surface it on the bus. When the run hit the
        // step limit with no answer, finalize_status also wrote a fallback final
        // answer + FinalAnswer event; mirror that with a Result delta.
        if !answered && run.final_answer.trim().is_empty() {
            // finalize_status left it empty only on a non-Complete terminal path;
            // nothing extra to emit beyond the status below.
        } else if !answered {
            emit_signal!(SignalKind::Result {
                final_text: run.final_answer.clone(),
            });
        }
        emit_signal!(SignalKind::StatusSet { status: run.status });
        ...
        snapshot.status = match run.status {
            ...
            RunStatus::Unverified => "Run unverified — the verifier gate did not pass.".to_string(),
        ...
        // This run has reached a terminal/paused state; drop its id from the keyed
        // interrupt set so a Stop that landed mid-run does not leak into the set.
        clear_interrupt(&RunId::from(run.id.clone()));
        // Free this run's captured output ring now that every phase (incl. verify) has
        // read it — keeps the per-run live-output buffers from accumulating across runs.
        output_capture::clear(&run.id);
        snapshot.set_current_run(Some(run.clone()));
...
where
    F: FnMut(AgentRun),
{
    // No global flag to clear: each run owns a fresh `RunId`, so its per-instance
    // interrupt entry is never pre-set. The run clears its own id from the keyed
    // set when it finishes (see `RunSession::run`).
    let agent_loop = RunSession::new(executor, &snapshot, &goal, &params);
    Ok(agent_loop.run(snapshot, goal, observer, signal).await)
}
...
fn finalize_status(run: &mut AgentRun, answered: bool, gate_satisfied: bool) {
    match run.status {
        RunStatus::Error | RunStatus::Interrupted | RunStatus::Paused => {}
        // The strategy has a verifier gate that never passed: the run is NOT a success.
        // Stamp it Unverified and fabricate no final answer — any verifier feedback is
        // already in run.messages. Closes the false-success paths (loop fall-off and a
        // budget-capped gate Back) that previously stamped Complete.
        _ if !gate_satisfied => {
...
pub(crate) fn pick_agent(snapshot: &AppSnapshot, requested: Option<&str>) -> Agent {
    ...
    // An un-targeted run goes to the orchestrator when one is enabled (the single entry
    // agent the user talks to, which delegates to the team), then to the first enabled
    // agent, then to anything at all.
    snapshot
        .agents
        .iter()
        .find(|agent| agent.enabled && agent.is_orchestrator)
        .or_else(|| snapshot.agents.iter().find(|agent| agent.enabled))
...
/// The title of the run-start routing row — the single source read by both
/// `build_run` (which pushes the legacy `AgentEvent`) and the strategy driver
/// (which emits the mirrored `SignalKind::Phase` so the bus carries the same
/// row). Keeping one source guarantees the two never drift apart.
fn routing_title(lane: RunLane) -> String {
...
mod tests {
    ...
    // The run-start routing row (pushed by `build_run`) and its mirrored Signal
    // (emitted by the driver so the bus carries the same row) must share ONE
    // title, or the bus event log diverges from the legacy log. `routing_title`
    // is the single source both read.
    #[test]
    fn routing_title_matches_the_run_start_event_label() {
        assert_eq!(routing_title(RunLane::BoundedTask), "Routing: bounded task");
        assert_eq!(
            routing_title(RunLane::SingleAction),
            "Routing: single action"
        );
        assert_eq!(routing_title(RunLane::Batch), "Routing: batch");
    }
    ...
    #[test]
    fn interrupt_flag_round_trips_per_instance() {
        let id = RunId::from("run-interrupt-test");
        let other = RunId::from("run-other-test");
        clear_interrupt(&id);
        assert!(!interrupt_requested(&id));
        request_interrupt(&id);
        assert!(interrupt_requested(&id));
        // The flag is per-instance: a second run's id is unaffected.
        assert!(!interrupt_requested(&other));
        clear_interrupt(&id);
        assert!(!interrupt_requested(&id));
    }

    #[test]
    fn pause_resume_flag_round_trips_per_instance() {
        let id = RunId::from("run-pause-test");
        resume(&id);
    ...
    fn finalize_status_preserves_paused_run() {
        ...
        finalize_status(&mut run, false, true);
    ...
    #[test]
    fn finalize_status_unverified_when_gate_unsatisfied() {
        let mut run = test_run_with_evidence();
        run.status = RunStatus::Running;
        run.final_answer.clear();
        let events_before = run.events.len();
        finalize_status(&mut run, false, false);
        // The verifier gate never passed: a non-success terminal, no fabricated answer
        // and no fabricated FinalAnswer event.
        assert_eq!(run.status, RunStatus::Unverified);
        assert!(run.final_answer.is_empty());
        assert_eq!(run.events.len(), events_before);
    }

    #[test]
    fn finalize_status_complete_when_gate_satisfied() {
        let mut run = test_run_with_evidence();
        run.status = RunStatus::Running;
        run.final_answer.clear();
        finalize_status(&mut run, false, true);
        // Satisfied (or gateless): legacy completion, including the step-limit fallback.
    ...
    fn delegate_phase_exposes_only_the_subset_tools() {
        ...
        use crate::strategy::{OrchestrateStrategy, Strategy};
    ...
    fn tool_policy_no_tools_and_inherit_bound_the_extremes() {
        ...
        assert_eq!(
            filter_tools_by_policy(&ToolPolicy::Inherit, &specs).len(),
            2
        );
    }
    ...
    #[test]
    fn tool_side_artifacts_reach_the_live_run_through_dispatch() {
        // Regression: a tool appends artifacts onto its own snapshot CLONE (the
        // capture pattern, e.g. `camera_capture::attach_image_artifact`). Before
        // the dispatch-merge fix those appends were dropped with the clone and
        // never reached the live `run` the hooks/observer read. Drive two
        // artifact-producing tool turns and assert both land, accumulated by id.
        use crate::state::{ArtifactKind, RunArtifact};

        // A tool that pushes one uniquely-id'd image artifact onto the run in the
        // snapshot it is handed — exactly what a real capture tool does.
        fn artifact_tool(id: &'static str) -> crate::core::ToolBinding {
            Rc::new(
                move |snapshot: &mut AppSnapshot, _args: &serde_json::Value| {
                    if let Some(run) = snapshot.current_run_mut() {
                        run.scratchpad.artifacts.push(RunArtifact {
                            id: id.to_string(),
                            name: format!("capture {id}"),
                            artifact_type: ArtifactKind::Image,
                            content: format!("data:image/png;base64,{id}"),
                        });
                    }
                    Box::pin(async move { Ok(format!("captured ({id})")) })
                },
            )
        }

        // Final answer quotes the run as evidence so the answer validator accepts
        // it (a bare "done" is rejected — that path is exercised elsewhere).
        let inference = ScriptedInference::new(vec![
            reply(ReActAction::Tool, "snap_a({})", "raw-1"),
            reply(ReActAction::Tool, "snap_b({})", "raw-2"),
            reply(
                ReActAction::Answer,
                "Captured two frames: I ran snap_a (captured (frame-a)) and snap_b \
                 (captured (frame-b)) and saw both succeed.",
                "raw-3",
            ),
        ]);
        let (agent_loop, mut engine, mut run, mut snapshot) =
            shell_fixture("capture two frames", Rc::clone(&inference));
        engine.base.tools.bind("snap_a", artifact_tool("frame-a"));
        engine.base.tools.bind("snap_b", artifact_tool("frame-b"));

        // The live run starts with no artifacts; the phase runner mirrors it into
        // the snapshot so the dispatch baseline is shared and correct.
        assert!(run.scratchpad.artifacts.is_empty());
        ...
        // Both tool-side artifacts reached the AUTHORITATIVE in-flight run, in
        // call order, accumulated (union by id) — not dropped with the clones.
        // (Asserted on the run regardless of the final-answer verdict; the tool
        // turns are what the dispatch-merge fix covers.)
        let ids: Vec<&str> = run
            .scratchpad
            .artifacts
            .iter()
        ...
        assert_eq!(
            ids,
            vec!["frame-a", "frame-b"],
            "tool-side artifacts must merge into the live run, once each"
        );
    }

    #[test]
    fn execution_tool_output_is_captured_into_the_run_output_ring() {
        // The live-output surface: an execution tool's result is captured into
        // this run's bounded ring so a later phase can read it back via
        // `read_run_output`. Bind a fake `run_js` and assert its output landed
        // keyed by the run id.
        let inference = ScriptedInference::new(vec![
            reply(ReActAction::Tool, "run_js({\"code\":\"x\"})", "raw-1"),
            reply(
                ReActAction::Answer,
                "I ran the check and saw PASS: add(2,3)==5 in the output.",
                "raw-2",
            ),
        ]);
        let (agent_loop, mut engine, mut run, mut snapshot) =
            shell_fixture("run the check", Rc::clone(&inference));
        super::output_capture::clear(&run.id);
        engine.base.tools.bind(
            "run_js",
            Rc::new(|_s, _a| Box::pin(async { Ok("PASS: add(2,3)==5\nexit 0".to_string()) })),
        );
        ...
        // The execution tool's stdout reached the run-keyed output ring; a verify
        // phase reading `read_run_output` for this run would see it.
        let captured = super::output_capture::tail(&run.id, 50);
        assert!(
            captured.contains("PASS: add(2,3)==5") && captured.contains("exit 0"),
            "execution-tool output must be captured into the run ring, got: {captured:?}"
        );
        super::output_capture::clear(&run.id);
    }
    ...
    fn interrupt_between_turns_marks_the_run_interrupted() {
        ...
        // Per-instance: flag THIS run's id, the same id the hooks consult.
        let run_id = RunId::from(run.id.clone());
        request_interrupt(&run_id);
        let phase = &agent_loop.strategy.phases()[0];
        let mut steps_used = 0u32;
        let mut last_answer = None;
        let outcome = run_to_completion(agent_loop.run_loop_phase(
