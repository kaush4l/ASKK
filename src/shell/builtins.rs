...
/// Dispatch one tokenized builtin with no stdin. Thin wrapper over
/// [`run_builtin_with_stdin`] for the unit tests, which have no piped input.
/// Production always routes through [`run_builtin_with_stdin`].
#[cfg(test)]
pub async fn run_builtin(
    ...
    run_builtin_with_stdin(session, fs, argv, "").await
}

/// Dispatch one tokenized builtin, threading `stdin` from a pipe or `<` redirect.
///
/// `argv` is non-empty and `argv[0]` is one of the names routed here by
/// [`super::run_line`]. Most builtins ignore `stdin`; `cat` with no file
/// arguments echoes it (so `echo hi | cat` and `cat < file` work).
pub async fn run_builtin_with_stdin(
    session: &mut ShellSession,
    fs: &ShellFs,
    argv: &[String],
    stdin: &str,
    ...
    match command.as_str() {
        ...
        "echo" => Ok(argv[1..].join(" ")),
        "ls" => ls(session, fs, &argv[1..]).await,
        "cat" => cat(session, fs, argv, stdin).await,
...
/// Whether `path` is a directory implied by the listing (explicit or ancestor).
/// Shared with the redirection guard in [`super`] so there is one definition.
pub fn path_is_dir(entries: &[(String, bool)], path: &str) -> bool {
    !path.is_empty() && implied_dirs(entries).contains(path)
}
...
fn help_text() -> String {
    [
        ...
        "  cat [file...]    print files (no args: echo stdin)",
        ...
        "  *  ?  [...]      glob-expand against the workspace listing",
    ]
    .join("\n")
}
...
async fn ls(session: &ShellSession, fs: &ShellFs, operands: &[String]) -> AppResult<String> {
    let entries = fs.list_all().await?;
    // No operands: list the cwd. One or more (e.g. from a glob): list each,
    // labelling directory blocks when there is more than one operand, like
    // POSIX `ls a b`.
    ...
    let mut blocks: Vec<String> = Vec::with_capacity(targets.len());
    for shown in &targets {
        blocks.push(ls_one(&session.cwd, &entries, shown, labelled)?);
    }
    Ok(blocks.join("\n"))
}

/// List one `ls` operand. A file operand prints its own name; a directory
/// operand prints its children (dirs first). With `labelled`, a directory's
/// listing is prefixed with `shown:` so multiple operands stay distinguishable.
fn ls_one(cwd: &str, entries: &[(String, bool)], shown: &str, labelled: bool) -> AppResult<String> {
    let target = resolve_path(cwd, shown)?;
    if !target.is_empty() && !implied_dirs(entries).contains(&target) {
        if entries
            .iter()
            .any(|(path, is_dir)| path == &target && !is_dir)
        {
            // A file operand: print it as typed (a glob already resolved it).
...
async fn cat(
    ...
    let files = &argv[1..];
    // No file arguments: echo stdin, the POSIX `cat` with no operands.
    if files.is_empty() {
    ...
    for arg in files {
        ...
        match fs.read_file(&path).await? {
            Some(content) => chunks.push(content),
            None => return Err(format!("cat: {arg}: no such file")),
        }
    }
    // Concatenate verbatim; a missing trailing newline on one file is not
    // synthesised, matching `cat`.
    Ok(chunks.join(""))
}
