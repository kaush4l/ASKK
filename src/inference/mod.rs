...
pub struct InferenceRequest {
    ...
    /// The active phase's framing line — `"{name}: {prompt_frame}"` — rendered into
    /// the system prompt's `## CURRENT PHASE` block (after `## CONTEXT`). Empty for a
    /// bare/unframed phase (e.g. the `react` strategy), keeping the prompt
    /// byte-identical to the pre-phase form. The engine fills this from the running
    /// [`Phase`](crate::strategy::Phase); providers never compute it.
    pub phase_context: String,
    /// The fully composed `## WORKSPACE` block — the agent's open files (with their
    /// fresh content), the folder tree, and the terminal tail — rendered into the
    /// system prompt after `## CONTEXT`. Assembled shell-side from OPFS + the
    /// live-output ring (the renderer is pure), refreshed each turn so the content is
    /// current. Empty when no file is open, keeping the prompt byte-identical to the
    /// pre-workspace form. The engine fills this; providers never compute it.
