...
thread_local! {
    ...
    /// Monotonic change-counter for the cache, satisfying the shared
    /// [`Registry::version`] contract. Bumped whenever a new key is cached;
    /// nothing on the live path reads it yet (the cache is keyed by content, not
    /// polled), so it is purely additive — `get_or_create`'s behavior is
    /// unchanged.
    static IMPL_VERSION: Cell<u64> = const { Cell::new(0) };
}
...
pub fn get_or_create(raw: &str) -> Rc<ProviderImpl> {
    ...
    IMPLEMENTATIONS.with(|cache| {
        ...
        // A new key entered the cache: advance the shared change-counter.
        IMPL_VERSION.with(|version| version.set(version.get().wrapping_add(1)));
        impl_handle
    })
}

/// A zero-sized handle onto the inference cache, giving it the shared
/// [`Registry`] vocabulary without changing the `get_or_create` cache path. The
/// cache itself stays a `thread_local!` `HashMap` (its across-call,
/// single-threaded-WASM lifecycle is unchanged); this handle is the seam's view
/// of it.
///
/// `Registry::get` here is a *pure* read — it never builds a missing impl (that
/// is `get_or_create`'s job). `insert` populates the cache directly, used by the
/// trait-level mental model and tests; the live path keeps calling
/// `get_or_create`.
pub struct InferenceCache;

impl Registry<String, Rc<ProviderImpl>> for InferenceCache {
    fn get(&self, key: &String) -> Option<Rc<ProviderImpl>> {
        IMPLEMENTATIONS.with(|cache| cache.borrow().get(key).map(Rc::clone))
    }

    fn insert(&mut self, key: String, value: Rc<ProviderImpl>) {
        let is_new = IMPLEMENTATIONS.with(|cache| cache.borrow_mut().insert(key, value).is_none());
        if is_new {
            IMPL_VERSION.with(|version| version.set(version.get().wrapping_add(1)));
        }
    }
    ...
    fn keys(&self) -> Vec<String> {
        IMPLEMENTATIONS.with(|cache| cache.borrow().keys().cloned().collect())
    }
...
/// Resets the change-counter too, so version-sensitive tests start from 0.
#[cfg(test)]
pub(crate) fn clear_cache() {
    IMPLEMENTATIONS.with(|cache| cache.borrow_mut().clear());
    IMPL_VERSION.with(|version| version.set(0));
}
...
mod tests {
    ...
    #[test]
    fn registry_handle_reads_same_cache_as_get_or_create() {
        clear_cache();
        let cache = InferenceCache;
        // Empty cache at version 0.
        assert_eq!(Registry::version(&cache), 0);
        assert!(cache.get(&"openai/gpt-4o-mini".to_string()).is_none());

        // A build via the live path is observable through the seam handle, as the
        // very same `Rc`.
        let built = get_or_create("openai/gpt-4o-mini");
        let via_handle = cache.get(&"openai/gpt-4o-mini".to_string());
        assert!(via_handle.is_some());
        assert!(Rc::ptr_eq(&built, &via_handle.unwrap()));

        // The change-counter advanced exactly once for the one new key, and the
        // handle reports the same key set / length as the cache.
        assert_eq!(Registry::version(&cache), 1);
        assert_eq!(Registry::len(&cache), cache_len());
        assert_eq!(
            Registry::keys(&cache),
