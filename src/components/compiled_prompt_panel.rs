...
pub fn CompiledPromptPanel(snapshot: Signal<AppSnapshot>) -> Element {
    ...
    rsx! {
        Card { class: "shell-side-panel shell-prompt-panel",
            div { class: "shell-panel-head",
                h2 { "Compiled prompt" }
                Badge { tone: "info", "{agent.name}" }
            }
            ...
            div { class: "shell-scroll",
                pre { class: "shell-prompt-pre", "{prompt}" }
            }
        }
    }
}
...
fn compile_preview_prompt(snapshot: &AppSnapshot, agent: &crate::state::Agent) -> String {
    ...
    let request = InferenceRequest {
        ...
        // Static preview: no active phase, so no `## CURRENT PHASE` block.
