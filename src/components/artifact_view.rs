...
pub fn ArtifactCard(artifact: RunArtifact) -> Element {
    ...
    rsx! {
        // Soft-desk card: the `ui-card` primitive gives the rounded surface +
        // light elevation; `artifact-card` carries the figure-specific grid
        // layout (page CSS in chat.css). Kept a <figure> for the caption semantics.
        figure { class: "ui-card artifact-card",
...
fn render_body(kind: ArtifactKind, content: &str) -> Element {
    match kind {
        ...
        // Captured run output renders as monospace text, same path as plain text.
        ArtifactKind::LiveOutput => render_text(content),
