        ...
        section { class: "panel page-panel agents-page",
            ...
            div { class: "new-agent-row",
                input {
                    ...
                    aria_label: "New agent name",
                    value: "{new_agent_name.read()}",
                    oninput: move |event| new_agent_name.set(event.value())
                }
                input {
                    placeholder: "Role — one-line purpose",
                    aria_label: "New agent role",
                    ...
                    article { class: "agent-card", key: "{agent.id}",
                        div { class: "agent-card-head",
                            ...
                            if let Some(path) = agent.source_path.as_ref() {
                                span { class: "registry-badge is-muted", "{path}" }
                            }
                            button {
                                class: "ghost-button",
                                onclick: move |_| {
                        ...
                        div { class: "muted", style: "font-size: 12px;", "Tools available to this agent — set in its definition" }
                        div { class: "tool-grid",
                            for tool_name in tool_names.iter() {
                                label {
                                    class: "checkbox-line tool-checkbox read-only",
                                    key: "{agent.id}-{tool_name}",
                                    title: "Tools are configured in the agent's Markdown definition",
                                    input {
                                        r#type: "checkbox",
