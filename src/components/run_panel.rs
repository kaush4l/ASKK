...
/// writes results into the interactive terminal via its inject queue. `code`
/// is the active buffer (possibly unsaved), so JS runs exactly what the
/// editor shows.
#[component]
pub fn RunButton(
    path: Option<String>,
            ...
            onclick: move |_| {
                ...
                match dispatch {
                    ...
                    RunDispatch::Js => {
                        ...
                        busy.set(true);
                        term_inject
                            .with_mut(|queue| queue.push(TermInject::Write(format!("> run {path}\n"))));
                    ...
                    RunDispatch::Python | RunDispatch::Wasm => {
                        // Shell argv convention: argv[0] is the command name,
                        // the file is argv[1] (see `shell::runtime::run_runtime`).
                        ...
                        spawn_local(async move {
                            ...
                            let response =
                                run_runtime(kind, &argv, &ShellExecCtx::default()).await;
                            term_inject.with_mut(|queue| {
