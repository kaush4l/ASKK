...
// `onSave` receives the format-on-save result: the bundle asks the attached
// language service to format the doc before reporting the save, so the text
// forwarded here (and persisted) is already formatted. Languages without a
// formatter save unchanged. `{ fold: true }` enables the code-folding gutter
// (the bundle's default; passed explicitly for clarity).
...
/// / completion / hover / publishDiagnostics, plus an optional `format`
/// request used for format-on-save); `languages` lists the language
