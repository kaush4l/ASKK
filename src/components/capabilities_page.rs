...
const REGISTRY_CSS: Asset = asset!("/assets/pages/registry.css");
...
pub fn CapabilitiesPage() -> Element {
    ...
    rsx! {
        document::Stylesheet { href: REGISTRY_CSS }
                                ...
                                div { class: "cap-grid",
                                    for entry in entries {
                                        div { class: "cap-card", key: "{entry.id}",
                                            span {
                                                class: cap_dot_class(entry.status),
                                                role: "img",
                                                aria_label: cap_dot_label(entry.status),
                                            }
                                            div { class: "cap-body",
...
/// Spoken status for the color/shape dot, so the state reaches screen readers
/// (the dot itself is decorative once labelled).
fn cap_dot_label(status: CapabilityStatus) -> &'static str {
    match status {
        CapabilityStatus::Yes => "supported",
        CapabilityStatus::Partial => "partial support",
        CapabilityStatus::No => "not supported",
    }
}
