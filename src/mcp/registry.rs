...
use crate::mcp::bridge_transport::BridgeMcpTransport;
...
use crate::registry::VersionedVec;
use crate::state::{
    AgentEventKind, AgentRun, AppResult, McpServerConfig, McpServerDefinition, McpServerKind,
    ProcessServerSpec, ToolConfig, ToolResult, ToolSpec, default_tool_names, event,
};
use dioxus::prelude::*;
...
/// Whether a server kind is connected at run start (and so brought up by the
/// in-browser runtime's bring-up filter). The browser/shellized/workspace kinds run
/// inside the tab; a `Process` kind connects out through the local bridge — not inside
/// the tab, but still a live connection the run depends on, so it must be brought up
/// too. Exhaustive (no wildcard) so a new kind forces an explicit decision here.
fn connected_at_run_start(kind: McpServerKind) -> bool {
    match kind {
        McpServerKind::Browser
        | McpServerKind::Shellized
        | McpServerKind::Workspace
        | McpServerKind::Process => true,
    }
}

thread_local! {
    /// Live connections behind the shared [`VersionedVec`] helper: the same
    /// owned-`Vec` store the registry always used, now with the seam's monotonic
    /// change-counter (bumped on connect / eviction). The connection set is keyed
    /// by `server_id`; lookups remain by namespaced display name.
    static MCP_RUNTIME: RefCell<VersionedVec<McpConnection>> =
        const { RefCell::new(VersionedVec::new()) };
}
...
/// that need it); a `Process` server is a real OS process spawned and relayed by the
/// local bridge (the tab can't spawn it). All end up behind the same boxed
/// [`McpTransport`], so everything downstream is identical.
async fn connect_server(
    ...
    let transport: Box<dyn McpTransport> = match config.kind {
        ...
        McpServerKind::Process => {
            let spec = ProcessServerSpec::parse(&config.definition)?;
            Box::new(
                BridgeMcpTransport::connect(&tool_config.web_search, config.id.clone(), spec)
                    .await?,
            )
        }
    };
    ...
    F: FnMut(AgentRun),
{
    ...
    let live_fingerprints: Vec<String> = servers
        .iter()
        .filter(|server| server.enabled && connected_at_run_start(server.kind))
