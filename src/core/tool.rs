...
use super::contract::HostBinding;
use super::tooling::{ToolBinding, ToolFuture};

/// Default per-call tool deadline (ms) for the placement contract until a tool
/// declares its own. Generous enough for an MCP round-trip; the hub supervises it.
pub const DEFAULT_TOOL_TIMEOUT_MS: u32 = 30_000;
...
pub trait Tool {
    ...
    /// Where this tool runs and what state it needs — its *placement* contract
    /// ([`HostBinding`]). The hub routes a tool call by this once dispatch is cut over
    /// (plan C3): a pure Rust tool runs in the engine worker, an MCP tool on its host,
    /// a page-op tool on main, an agent-as-tool via delegation. The default is the
    /// common case — a pure in-worker tool needing nothing — so existing impls keep
    /// compiling unchanged; environment-bound paradigms override it.
    fn binding(&self) -> HostBinding {
        HostBinding::in_worker(DEFAULT_TOOL_TIMEOUT_MS)
    }
