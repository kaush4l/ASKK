...
use crate::state::{
    AgentMemory, AppResult, AppSnapshot, Message, ProviderConfig, RunArtifact, Skill, ToolResult,
    ToolSpec, WorkspaceView, merge_agent_memories, now_iso,
};
...
pub trait EngineHooks {
    ...
    /// Compose the `## WORKSPACE` block body for the turn about to render — the
    /// folder tree, each open file's *fresh* content, and the terminal tail. The
    /// engine calls this just before [`Engine::render`] and stashes the result on
    /// [`BaseEngine::workspace_context`] (the renderer is pure, so the async OPFS
    /// reads happen here, shell-side). Default: `""` — no workspace block, the
    /// prompt stays byte-identical to legacy. The shell reads the run's open-set and
    /// builds the block; the bare loop (and `NoHooks`) renders nothing.
    #[allow(async_fn_in_trait)]
    ...
    /// A streaming partial answer arrived mid-generation: `partial` is the parsed
    /// displayable text so far (from the streaming parser), not a raw token chunk.
    /// Default: ignore. The shell emits an `LlmDelta` so the UI shows the reply
    /// forming live.
    ...
    /// Tool-side artifacts surfaced for one settled call, fired **before** the
    /// matching [`Self::on_tool_finished`] so the shell can union them into the
    /// authoritative in-flight run and emit an append delta for each. A tool
    /// runs against its own `snapshot` clone (see [`Engine::dispatch_tools`]);
    /// any [`RunArtifact`](crate::state::RunArtifact) it pushes onto that clone's
    /// run would otherwise be dropped. The default no-op keeps the bare loop
    /// artifact-free; the shell merges by artifact id.
    fn on_artifacts_appended(&mut self, artifacts: Vec<crate::state::RunArtifact>) {
        let _ = artifacts;
    }

    /// The agent's workspace open-set changed for one settled call — it opened or
    /// closed a file via `workspace_open`/`workspace_close`. Fired **before** the
    /// matching [`Self::on_tool_finished`], on the same lift-from-the-clone path as
    /// [`Self::on_artifacts_appended`]: the tool mutates `scratchpad.workspace` on
    /// its own `snapshot` clone (see [`Engine::dispatch_tools`]), so the shell folds
    /// the new view onto the authoritative in-flight run here and emits a
    /// `WorkspaceChanged` delta. The default no-op keeps the bare loop workspace-free.
...
pub struct BaseEngine {
    ...
    /// The active phase's framing line — `"{name}: {prompt_frame}"` — copied into
    /// each rendered [`InferenceRequest`] so the system prompt's `## CURRENT PHASE`
    /// block names the phase. Set per-phase by the shell alongside `response_kind`;
    /// empty for a bare/unframed phase (keeps the prompt byte-identical to legacy).
    pub phase_context: String,
    /// The fully composed `## WORKSPACE` block for the next render — the folder
    /// tree, each open file's *fresh* content, and the terminal tail, all assembled
    /// shell-side (OPFS reads are async/platform-bound, the renderer is pure). Empty
    /// when no file is open (keeps the prompt byte-identical to legacy). Refreshed
    /// each turn by the shell's `before_turn` hook so the content is always current.
...
pub trait Engine {
    ...
    fn render(&self, goal_text: &str) -> InferenceRequest {
        ...
        InferenceRequest {
            ...
            phase_context: base.phase_context.clone(),
            workspace_context: base.workspace_context.clone(),
    ...
    async fn call_model<H: EngineHooks>(
        ...
        let output = loop {
            attempt += 1;
            let result = {
                // Stream the reply: each parsed partial answer (from the streaming
                // parser) goes to the delta hook so the shell surfaces it live. The
                // sink is scoped to this attempt so it only borrows `hooks` for the
                // streaming call, leaving `hooks` free for the failure/exhaustion
                // calls below.
                let mut sink = |partial: String| hooks.on_model_delta(&partial);
                let base = self.base();
                base.inference
    ...
    async fn dispatch_tools<H: EngineHooks>(
        ...
        // Tool-side artifacts (e.g. a camera/screenshot capture) land on the
        // call's `snapshot` clone, not the live run the hooks read. Snapshot the
        // pre-dispatch artifact set so each call's *new* artifacts can be lifted
        // out below and merged back through the hooks — without this they were
        // silently dropped with the clone.
        let baseline_artifacts: usize = snapshot
            .current_run()
            .map(|run| run.scratchpad.artifacts.len())
            .unwrap_or(0);

        // The pre-dispatch workspace open-set. A `workspace_open`/`workspace_close`
        // tool mutates `scratchpad.workspace` on its own snapshot clone (same lift
        // problem as artifacts); capturing the baseline lets each call report only
        // the view it actually changed so the shell can merge it back and emit one
        // `WorkspaceChanged` delta. `None` when nothing differs.
        let baseline_workspace: WorkspaceView = snapshot
            .current_run()
        ...
        let dispatched: Vec<DispatchOutput> = {
            let this = &*self;
            let futures = prepared.iter().map(|(call_id, call)| {
                ...
                let baseline_workspace = baseline_workspace.clone();
                async move {
                    let result = this
                        .execute_tool(&mut call_snapshot, call_id, &name, &args)
                        .await;
                    // Only the artifacts THIS call appended past the shared
                    // baseline — the clone carries the run's prior artifacts too.
                    let new_artifacts = call_snapshot
                        .current_run()
                        .map(|run| run.scratchpad.artifacts[baseline_artifacts..].to_vec())
                        .unwrap_or_default();
                    // The workspace view only if THIS call changed it from baseline.
                    let changed_workspace = call_snapshot
                        .current_run()
                        .map(|run| run.scratchpad.workspace.clone())
                        .filter(|view| *view != baseline_workspace);
                    (
                        result,
                        call_snapshot.agent_memories,
                        new_artifacts,
                        changed_workspace,
                    )
                }
        ...
        let mut artifact_batches: Vec<Vec<RunArtifact>> = Vec::with_capacity(dispatched.len());
        let mut workspace_batches: Vec<Option<WorkspaceView>> =
            Vec::with_capacity(dispatched.len());
        for (result, memories, artifacts, workspace) in dispatched {
            results.push(result);
            memory_batches.push(memories);
            artifact_batches.push(artifacts);
            workspace_batches.push(workspace);
        }

        // Accumulator the parallel workspace changes fold into, starting from the
        // shared pre-dispatch open-set. Each `workspace_open`/`workspace_close` call
        // diffed the SAME baseline on its own snapshot clone, so replacing the live
        // view with one call's whole result would clobber its siblings; folding their
        // per-call deltas onto this accumulator keeps every open/close (the same
        // union-merge the artifact lift uses).
        let mut merged_workspace = baseline_workspace.clone();
        let mut aborted = false;
        for ((((_, call), result), artifacts), workspace) in prepared
            .iter()
            .zip(results)
            .zip(artifact_batches)
            .zip(workspace_batches)
        {
            // Merge this call's artifacts into the authoritative in-flight run
            // BEFORE `on_tool_finished` rules on the result, so the shell's
            // artifact diff (and its `ArtifactAppended` emit) sees them as part
            // of this call. A no-op for the common case of zero artifacts.
            if !artifacts.is_empty() {
                hooks.on_artifacts_appended(artifacts);
            }
            // Fold this call's workspace change onto the accumulator, then hand the
            // shell the cumulative view (same ordering: before the result is ruled
            // on). A no-op when the call left the open-set alone.
            if let Some(view) = workspace {
                merged_workspace.fold_parallel_change(&baseline_workspace, &view);
                hooks.on_workspace_changed(merged_workspace.clone());
            }
            match hooks.on_tool_finished(&call.name, &result) {
                ...
                ToolVerdict::Reject { feedback } => {
                    // Tool output is DATA: record what the tool actually returned as
                    // the observation (the model's internal state) even when the shell
                    // rejects it, THEN add the critique as feedback. Showing only the
                    // feedback left the model re-deciding the next turn with no record
                    // of the result, so a tool that keeps failing or returning thin
                    // data (e.g. an in-browser news fetch) looped forever, re-calling
