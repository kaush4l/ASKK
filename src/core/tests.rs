...
#[test]
fn final_turn_injects_an_answer_now_nudge() {
    // Two-turn budget. Turn 1's answer is rejected (still gathering); turn 2 is
    // the final allowed turn and must carry the "answer now" nudge so the model
    // synthesizes instead of the run dying at the budget with no answer.
    let mock = MockInference::scripted(vec![
        answer_reply("partial", "raw-1"),
        answer_reply("final", "raw-2"),
    ]);
    let mut engine = engine_with(Rc::clone(&mock), 2);
    let mut snapshot = AppSnapshot::default();
    let mut hooks = RecordingHooks {
        answer_verdicts: VecDeque::from(vec![
            AnswerVerdict::Reject {
                feedback: "need more".to_string(),
            },
            AnswerVerdict::Accept,
        ]),
        ..Default::default()
    ...
    // The nudge is absent from the first request and present on the final turn.
    let first = mock.request(0);
    let final_turn = mock.request(1);
    assert!(
        !first
            .history
            .iter()
            .any(|m| m.content.contains("Final step"))
    );
    assert!(
        final_turn
            .history
            .iter()
            .any(|m| m.role == "user" && m.content.contains("Final step"))
    );
}

#[test]
fn single_step_budget_is_not_nudged() {
    // A one-turn budget has no prior turn to fold, so no nudge is injected.
    let mock = MockInference::scripted(vec![answer_reply("only", "raw-1")]);
    let mut engine = engine_with(Rc::clone(&mock), 1);
    let mut snapshot = AppSnapshot::default();
    let mut hooks = RecordingHooks::default();

    let outcome = run_to_completion(engine.invoke("goal", &mut snapshot, &mut hooks));

    assert_eq!(outcome.answer.as_deref(), Some("only"));
    let first = mock.request(0);
    assert!(
        !first
            .history
...
#[test]
fn dispatch_rejected_result_still_records_the_observation_then_feedback() {
    // The intended invariant: a tool call's returned DATA is appended to the
    // conversation (the model's internal state) even when validation REJECTS it,
    // then the critique follows. Dropping the data and showing only feedback left
    // the model re-deciding the next turn with no record of what the tool returned —
    // so a tool that keeps failing or returning thin data (e.g. an in-browser news
    // fetch) loops, re-calling forever with no new information.
    let mock = MockInference::scripted(vec![]);
    let mut engine = engine_with(mock, 4);
    engine.base.tools.bind(
        "news",
        Rc::new(|_s, _a| Box::pin(async { Ok("headline: rust 2.0 ships".to_string()) })),
    );
    let mut snapshot = AppSnapshot::default();
    let calls = vec![crate::responses::ParsedToolCall {
        name: "news".to_string(),
        args: json!({}),
    }];
    let mut hooks = RecordingHooks {
        tool_verdicts: VecDeque::from(vec![ToolVerdict::Reject {
            feedback: "needs a citation".to_string(),
        }]),
        ..Default::default()
    };
    ...
    assert!(
        continued,
        "a reject continues the loop (only abort stops it)"
    );
    // The DATA is recorded as the observation FIRST (internal state)…
    assert_eq!(
        engine.base.history[0],
        message("tool", "news -> headline: rust 2.0 ships")
    );
    // …then the critique enters as feedback.
    assert_eq!(engine.base.history[1], message("user", "needs a citation"));
    assert_eq!(engine.base.history.len(), 2);
}
