...
impl CompiledPrompt {
    ...
    /// `## CONTEXT` block carrying `now` (the current date) and the sandbox note, then —
    /// when the active phase declares a frame — a `## CURRENT PHASE` block naming the
    /// phase and its framing. This is the system prompt only; the response-format
    /// instructions are appended separately (see [`Self::render`]).
    ///
    /// Byte-parity: an empty `phase_context` AND an empty `workspace_context`
    /// produce exactly the legacy text (no `## WORKSPACE` / `## CURRENT PHASE`
    /// block), so non-phased / non-coder agents see no change.
    pub fn render_system(&self, now: &str, workspace_context: &str, phase_context: &str) -> String {
        let context = render_context(now);
        let workspace = render_workspace_context(workspace_context);
        let phase = render_phase_context(phase_context);
        format!("{}\n\n{context}{workspace}{phase}", self.static_body)
    }
...
pub fn render_system_prompt(request: &InferenceRequest) -> AppResult<String> {
    Ok(CompiledPrompt::from_request(request).render_system(
        &request.now,
...
/// The `## WORKSPACE` block, rendered after `## CONTEXT` and before
/// `## CURRENT PHASE` only when the agent has a non-empty workspace. `workspace`
/// is the pre-composed block body the shell assembles from OPFS (the folder tree,
/// each open file's fresh content, the terminal tail); empty ⇒ the empty string, so
/// the system prompt is byte-identical to the pre-workspace form. Composing it
/// shell-side keeps this renderer pure (OPFS reads are async/platform-bound).
fn render_workspace_context(workspace: &str) -> String {
...
/// The `## CURRENT PHASE` block, rendered after `## CONTEXT` only when the active phase
/// declared a frame. `phase_context` is the pre-composed `"{name}: {prompt_frame}"`
/// line the engine builds from the running [`Phase`](crate::strategy::Phase); empty ⇒
/// the empty string, so the system prompt is byte-identical to the unphased form.
fn render_phase_context(phase_context: &str) -> String {
...
fn describe_tools(tools: &[ToolSpec]) -> String {
    ...
    let entries = tools
        ...
        .join("\n\n");
    format!(
        "## AVAILABLE TOOLS\n\nThese are the only tools you can call. Calling any \
         other name (even one mentioned in your instructions) is rejected and \
         wastes a step — do not invoke a tool that is not listed here.\n\n{entries}"
    )
}
...
mod tests {
    ...
    #[test]
    fn tool_manifest_states_the_list_is_exhaustive() {
        let mut request = base_request();
        request.tools = vec![ToolSpec {
            name: "web_search".to_string(),
            description: "Search the web.".to_string(),
        ...
        let system = render_system_prompt(&request).unwrap();
        // The constraint that stops the model from calling unlisted tools.
        assert!(system.contains("These are the only tools you can call."));
        // The constraint sits under the header, before the per-tool entries.
        let header = system.find("## AVAILABLE TOOLS").unwrap();
        let constraint = system.find("These are the only tools").unwrap();
        let entry = system.find("## web_search").unwrap();
        assert!(header < constraint && constraint < entry);
    }
    ...
    #[test]
    fn phase_context_injects_a_current_phase_block_after_context() {
        let request = full_request();
        let compiled = CompiledPrompt::from_request(&request);

        // No phase frame ⇒ byte-identical to the bare system prompt (the parity
        // invariant the golden test pins).
        let bare = compiled.render_system(&request.now, "", "");
        assert!(!bare.contains("## CURRENT PHASE"));

        // A phase frame appends a `## CURRENT PHASE` block AFTER `## CONTEXT`.
        let phased = compiled.render_system(
            &request.now,
            "",
            "verify: RUN the tests yourself and judge the real output.",
        );
        let context_at = phased.find("## CONTEXT").unwrap();
        let phase_at = phased.find("## CURRENT PHASE").unwrap();
        assert!(context_at < phase_at, "phase block must follow context");
        assert!(phased.contains("verify: RUN the tests yourself"));
        // The phased prompt is the bare one plus exactly the appended block.
        assert_eq!(
            phased,
            format!(
                "{bare}\n\n## CURRENT PHASE\n\nverify: RUN the tests yourself and judge the real output."
    ...
    #[test]
    fn workspace_context_injects_a_workspace_block_between_context_and_phase() {
        let request = full_request();
        let compiled = CompiledPrompt::from_request(&request);

        // No workspace ⇒ byte-identical to the bare system prompt (parity invariant).
        let bare = compiled.render_system(&request.now, "", "");
        assert!(!bare.contains("## WORKSPACE"));

        // A workspace block lands AFTER `## CONTEXT` and BEFORE `## CURRENT PHASE`.
        let workspace_body = "src/lib.rs\n  open: src/lib.rs\n```\nfn main() {}\n```";
        let withws =
            compiled.render_system(&request.now, workspace_body, "execute: build the thing.");
        let context_at = withws.find("## CONTEXT").unwrap();
        let workspace_at = withws.find("## WORKSPACE").unwrap();
        let phase_at = withws.find("## CURRENT PHASE").unwrap();
        assert!(context_at < workspace_at, "workspace must follow context");
        assert!(workspace_at < phase_at, "workspace must precede phase");
        assert!(withws.contains("fn main() {}"));

        // Workspace-only (no phase) is the bare prompt plus exactly the block.
        let ws_only = compiled.render_system(&request.now, workspace_body, "");
        assert_eq!(
            ws_only,
            format!("{bare}\n\n## WORKSPACE\n\n{workspace_body}")
        );
