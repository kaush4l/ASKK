...
mod core;
mod engine;
// The typed error surface ([`error::EngineError`]) is introduced additively and
// adopted first in the tool layer, which lowers it to the `String` alias at its
// public boundary. The variants/helpers not yet referenced from a converted
// `Result<_, EngineError>` signature are reserved for the per-instance error
// panels that later units render; the bin-target lints can't see that future use.
#[allow(dead_code)]
mod error;
mod inference;
mod mcp;
// The shared registry seam: a `Registry` trait the four keyed lookups satisfy,
// plus a `VersionedVec` helper the process registry is built on. The trait is a
// vocabulary other registries implement (not all of its methods are reached on
// the bin target), so the lints can't see every use.
#[allow(dead_code)]
mod registry;
mod responses;
// The runtime bus + projection are a self-contained foundation; nothing on the
// live run path reads them yet (that wiring lands in a later step), so the
// bin-target lints can't see any use. Narrowed once a panel subscribes.
#[allow(dead_code, unused_imports)]
mod runtime;
mod scheduler;
mod shell;
