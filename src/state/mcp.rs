...
//!
//! [`McpServerKind::Process`] is the bridged-stdio kind: a real OS process (e.g. a
//! Node stdio MCP server like `chrome-devtools-mcp`) that a browser tab can't spawn
//! itself. A local bridge spawns the process from its [`ProcessServerSpec`]
//! (`command`/`args`/`env`/`cwd`, held in [`McpServerConfig::definition`]) and relays
//! JSON-RPC over stdio. Remote (HTTP) transports are added later — this enum is the
//! seam.
...
pub enum McpServerKind {
    ...
    Workspace,
    /// A real OS process speaking MCP over stdio (e.g. a Node stdio server like
    /// `chrome-devtools-mcp`). A browser tab can't spawn it, so a local bridge spawns
    /// the process from the [`ProcessServerSpec`] held in
    /// [`McpServerConfig::definition`] and relays JSON-RPC over stdio.
    Process,
}
...
impl McpServerConfig {
    ...
    /// Create a new, enabled [`McpServerKind::Process`] server with a fresh id. The
    /// `definition` carries the serialized [`ProcessServerSpec`] JSON
    /// (`command`/`args`/`env`/`cwd`) the local bridge spawns at run start;
    /// [`default_process_definition`] is a ready-to-edit starting point.
    pub fn new_process(
        name: impl Into<String>,
        command: impl Into<String>,
        args: Vec<String>,
    ) -> Self {
        let spec = ProcessServerSpec {
            command: command.into(),
            args,
            env: Vec::new(),
            cwd: None,
        };
        let definition = serde_json::to_string(&spec).unwrap_or_default();
        Self {
            id: Uuid::new_v4().to_string(),
            name: name.into(),
            kind: McpServerKind::Process,
            module_path: String::new(),
            definition,
            enabled: true,
        }
...
/// How to spawn a [`McpServerKind::Process`] server. Parsed from
/// [`McpServerConfig::definition`] and handed to the local bridge, which spawns the
/// `command` (with `args`, `env`, and optional working directory `cwd`) and relays
/// MCP JSON-RPC over its stdio.
// Built and parsed by the wasm registry + bridge + host tests; the non-test host bin
// never names it, so it reads as dead code there (see the note on `ShellToolDef`).
#[allow(dead_code)]
#[derive(Clone, Debug, Serialize, Deserialize, PartialEq, Default)]
pub struct ProcessServerSpec {
    /// The executable to spawn (e.g. `npx`). Must be non-empty.
    pub command: String,
    /// Arguments passed to the command (e.g. `["chrome-devtools-mcp@latest"]`).
    #[serde(default)]
    pub args: Vec<String>,
    /// Extra environment variables as `(key, value)` pairs, layered onto the bridge's
    /// environment.
    #[serde(default)]
    pub env: Vec<(String, String)>,
    /// Optional working directory for the spawned process. `None` inherits the
    /// bridge's cwd.
    #[serde(default)]
    pub cwd: Option<String>,
}

impl ProcessServerSpec {
    /// Parse and validate a process server spec from its JSON text, returning a
    /// human-readable error (surfaced in the UI / run log) on malformed input or an
    /// empty `command`.
    #[allow(dead_code)] // called by the wasm registry + bridge + host tests; see struct note.
    pub fn parse(text: &str) -> AppResult<Self> {
        let trimmed = text.trim();
        if trimmed.is_empty() {
            return Err("Process server spec is empty — set a command to run.".to_string());
        }
        let spec: ProcessServerSpec = serde_json::from_str(trimmed)
            .map_err(|err| format!("Invalid process server JSON: {err}"))?;
        if spec.command.trim().is_empty() {
            return Err("Process server spec has no command to run.".to_string());
        }
        Ok(spec)
    }
}

/// A ready-to-edit process server spec: spawns `npx chrome-devtools-mcp@latest` (the
/// Chrome DevTools stdio MCP server) with no extra env and the bridge's working
/// directory. New process servers start from this so the author has a working example
/// to adapt.
pub fn default_process_definition() -> String {
    let spec = json!({
        "command": "npx",
...
mod tests {
    ...
    #[test]
    fn process_config_round_trips_through_json() {
        let config = McpServerConfig::new_process(
            "Chrome DevTools",
            "npx",
            vec!["chrome-devtools-mcp@latest".to_string()],
        );
        assert_eq!(config.kind, McpServerKind::Process);
        let json = serde_json::to_string(&config).unwrap();
        let decoded: McpServerConfig = serde_json::from_str(&json).unwrap();
        assert_eq!(config, decoded);
    }
    ...
    #[test]
    fn process_spec_parses_default_partial_and_rejects_bad_input() {
        // The default template parses and exposes the npx command.
        let spec = ProcessServerSpec::parse(&default_process_definition())
            .expect("default process template is valid");
        assert_eq!(spec.command, "npx");
        assert_eq!(spec.args, vec!["chrome-devtools-mcp@latest".to_string()]);

        // Partial JSON (only `command`) deserializes, defaulting the rest.
        let partial = ProcessServerSpec::parse(r#"{ "command": "node" }"#).expect("partial parse");
        assert_eq!(partial.command, "node");
        assert!(partial.args.is_empty());
        assert!(partial.env.is_empty());
        assert_eq!(partial.cwd, None);

        // Empty text, missing command, and a whitespace-only command are all rejected.
        assert!(ProcessServerSpec::parse("   ").is_err());
        assert!(ProcessServerSpec::parse(r#"{ "args": [] }"#).is_err());
        assert!(ProcessServerSpec::parse(r#"{ "command": "   " }"#).is_err());
        assert!(ProcessServerSpec::parse("{ not json").is_err());
    }

    #[test]
    fn default_process_definition_parses_and_exposes_npx() {
        let spec = ProcessServerSpec::parse(&default_process_definition())
            .expect("default process definition is valid");
        assert_eq!(spec.command, "npx");
    }
    ...
    fn mcp_server_kind_serializes_snake_case() {
        ...
        assert_eq!(
            serde_json::to_string(&McpServerKind::Process).unwrap(),
            "\"process\""
        );
    }
