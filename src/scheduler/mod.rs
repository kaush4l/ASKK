...
mod runtime {
    ...
    use super::spawner::enqueue_agent_run;
    ...
    fn fire_entry(entry: &ScheduleEntry, snapshot: Signal<AppSnapshot>) {
        match &entry.payload {
            ...
            SchedulePayload::AgentRun { agent_id, goal } => {
                ...
                // Route through the single bounded enqueue seam: the scheduled run
                // becomes a first-class engine instance on the same worker path as
                // a chat/workspace submit, under the shared concurrency bound. The
                // spawner owns the live projection, persistence, and slot release.
                enqueue_agent_run(snapshot, Some(agent_id.clone()), goal.clone());
            }
        }
    }
