...
//! the optional dev-bridge file sync ([`workspace_files`]), and the
//! backend-agnostic file-IO abstraction ([`workspace_fs`]) that the
//! watcher-supervised runtime hands to tools so file bytes never cross the hub.
...
impl IndexedDbStorage {
    pub async fn open() -> AppResult<Self> {
        // DB version stays at 2: the object-store layout is unchanged. The whole
        // `AppSnapshot` is serialized as one value under `SNAPSHOT_KEY`, so the
        // multi-instance fleet schema is a pure serde change (a new `instances`
        // field) migrated entirely in `AppSnapshot::with_profile_defaults` at load
        // time — an absent `instances` array falls back to the legacy `current_run`.
        // No upgrade transaction is needed, so no version bump.
