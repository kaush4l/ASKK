...
///
/// `pub(crate)` so the [`workspace_fs`](crate::storage::workspace_fs) abstraction
/// shares this one definition instead of forking a second path validator — there
/// must be exactly one rule for what a workspace-relative path is.
pub(crate) fn validate_path(path: &str) -> AppResult<Vec<String>> {
    if path.is_empty() {
