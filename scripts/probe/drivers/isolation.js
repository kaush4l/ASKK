...
export async function run({ engine, launch, base, mode, log }) {
  ...
  page.on('requestfailed', (r) =>
    noise.push(`[requestfailed] ${r.url().slice(0, 90)} :: ${r.failure()?.errorText}`),
  )
  ...
  try {
    ...
    cell.first_nav = {
      status: nav.status(),
      coep_on_wire: nav.headers()['cross-origin-embedder-policy'] ?? '(absent)',
    }
    ...
    log(
      `FIRST NAV IN-PAGE (before any reload settles): crossOriginIsolated=${firstPaint.coi} SharedArrayBuffer=${firstPaint.sab}`,
    )
    ...
    log(
      `HARD RELOAD: nav_status=${cell.after_reload.status} crossOriginIsolated=${cell.after_reload.coi} sw_controller=${cell.after_reload.controller}`,
    )
    ...
    await p2
      .waitForFunction(() => window.__RESULTS?.done === true, null, { timeout: 120000 })
      .catch(() => {})
    const cold1 = await p2.evaluate(() => ({
      coi: self.crossOriginIsolated,
      reloads: window.__RESULTS?.reloads_needed,
    }))
    cell.cold_visit = {
      isolated_at_first_load: cold0,
      after_sw_install: cold1.coi,
      reloads_needed: cold1.reloads,
      navigations: navs.length,
    }
    log(
      `COLD FIRST VISIT: isolated_at_first_load=${cold0} -> after_sw_install=${cold1.coi} reloads_needed=${cold1.reloads} navigations=${navs.length}`,
    )
  } finally {
    cell.noise = noise
    if (noise.length) {
