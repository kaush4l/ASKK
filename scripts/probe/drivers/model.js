...
export async function run({ engine, launch, base, mode, echo, local, idleSeconds = 30, log }) {
  ...
  page.on('requestfailed', (r) =>
    noise.push(
      `[requestfailed] ${r.method()} ${r.url().slice(0, 72)} :: ${r.failure()?.errorText}`,
    ),
  )
  ...
  if (engine === 'chromium') {
    ...
    s.on('Network.requestWillBeSent', (e) => {
      if (e.type === 'Preflight' || e.request.method === 'OPTIONS')
        cdp.push({ k: 'preflight-sent', url: e.request.url.slice(0, 70) })
    })
  ...
  try {
    ...
    log(
      `404 CONTROL: status=${cell.control_404.status} server=${cell.control_404.server} coep=${cell.control_404.coep} coop=${cell.control_404.coop} corp=${cell.control_404.corp}`,
    )
    ...
    let settled = true
    await page
      .waitForFunction(() => window.__SETTLED === true, null, { timeout: 90000 })
      .catch((e) => {
        settled = false
    ...
    log(
      `ENFORCEMENT CONTROL (cross-origin no-CORP <img> python.org): ${JSON.stringify(cell.controls.enforcement_nocorp_img)}`,
    )

    for (const which of CALLS) {
      const r = await page
        .evaluate((w) => self.REAL_CALLS[w](), which)
        .catch((e) => ({ driver_error: String(e).slice(0, 200) }))
      cell[which] = r
    ...
    echo.reset()
    cell.echo = await page
      .evaluate(() => self.REAL_CALLS.echo())
      .catch((e) => ({ driver_error: String(e).slice(0, 200) }))
    log(`ECHO (CORP-less, preflighted, SSE) ${JSON.stringify(cell.echo)}`)
    cell.echo_server_saw = echo
      .read()
      ...
      .map((r) => ({
        ...
        acrm: r.acrm,
        acrh: r.acrh,
        sec_fetch_mode: r.sec_fetch_mode,
        custom: r.custom,
      }))
    ...
    if (idleSeconds > 0) {
      ...
      cell.local_long_aged = await page
        .evaluate(() => self.REAL_CALLS.local_long())
        .catch((e) => ({ driver_error: String(e).slice(0, 200) }))
      log(
        `AGED local_long (after ${idleSeconds}s isolated) ${JSON.stringify(cell.local_long_aged)}`,
      )
    }
