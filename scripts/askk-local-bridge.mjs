...
export function createBridgeServer(rawOptions = {}) {
    ...
    const mcpPath = normalizePath(rawOptions.mcpPath ?? "/askk/mcp");
    ...
    const server = http.createServer(async (request, response) => {
        ...
        try {
            ...
            if (incoming.pathname.startsWith(`${mcpPath}/`)) {
                await handleMcpRequest(request, response, mcpPath, execConfig);
                return;
            }
...
export function startBridge(rawOptions = {}) {
    ...
    bridge.server.listen(port, host, () => {
        ...
        console.log(`http://${host}:${port}/askk/mcp/spawn | send | kill`);
    });
...
// Module-level registry of live process-backed MCP servers, keyed by caller id.
// Each entry holds the child process, a stdout line buffer, and a map of
// in-flight JSON-RPC requests awaiting their correlated response.
const mcpServers = new Map();
const MCP_DEFAULT_TIMEOUT_MS = 30_000;
const MCP_NOTIFICATION_RING = 32;

async function handleMcpRequest(request, response, mcpPath, execConfig) {
    if (request.method !== "POST") {
        writeJson(response, 405, { success: false, error: "MCP endpoints require POST." });
        return;
    }

    const incoming = new URL(request.url ?? "/", "http://bridge.local");
    const action = incoming.pathname.slice(`${mcpPath}/`.length);
    const body = await readJsonBody(request);

    if (action === "spawn") {
        const result = await mcpSpawn(body, execConfig);
        writeJson(response, result.success === false ? 400 : 200, result);
        return;
    }
    if (action === "send") {
        const result = await mcpSend(body, execConfig);
        writeJson(response, result.success === false ? 400 : 200, result);
        return;
    }
    if (action === "kill") {
        const result = mcpKill(body);
        writeJson(response, result.success === false ? 400 : 200, result);
        return;
    }

    writeJson(response, 404, { success: false, error: `Unknown ASKK MCP route: ${action}` });
}

export async function mcpSpawn(body, execConfig = {}) {
    if (!execConfig.allowExec) {
        return {
            success: false,
            error:
                "Command execution is disabled. Restart the ASKK local bridge with --allow-exec (or ASKK_ALLOW_EXEC=1) to let the agent spawn process-backed MCP servers in the run root.",
        };
    }

    const id = requiredString(body, "id");
    const command = requiredString(body, "command");
    if (!ALLOWED_EXEC_BINARIES.has(command)) {
        return {
            success: false,
            error: `mcp spawn blocked: '${command}' is not in the allowed binary list. Allowed: ${[...ALLOWED_EXEC_BINARIES].join(", ")}.`,
        };
    }
    ...
    try {
        ...
        return { success: false, error: `mcp spawn invalid cwd: ${error.message}` };
    }

    const env = { ...process.env };
    if (Array.isArray(body.env)) {
        for (const pair of body.env) {
    ...
    // A config edit re-spawns the same id; tear down the previous child first.
    const existing = mcpServers.get(id);
    if (existing) {
        teardownMcpServer(id, existing, "replaced by a new spawn");
    }
    ...
    try {
        ...
        return { success: false, error: `mcp spawn failed to start '${command}': ${error.message}` };
    }

    const entry = { child, buffer: "", pending: new Map(), notifications: [] };
    mcpServers.set(id, entry);

    // Guard against an async EPIPE when the child closes stdin under us; an
    // unhandled stream 'error' would otherwise crash the whole bridge.
    child.stdin.on("error", (error) => {
        console.error(`[askk-mcp:${id}] stdin error: ${error.message}`);
    });
    ...
    child.stderr.on("data", (chunk) => {
        for (const line of String(chunk).split(/\r?\n/)) {
            if (line.trim()) {
                console.error(`[askk-mcp:${id}] ${line}`);
            }
        }
    });

    child.on("error", (error) => {
        rejectMcpPending(entry, `MCP server '${id}' process error: ${error.message}`);
        if (mcpServers.get(id) === entry) {
            mcpServers.delete(id);
        }
    });
    child.on("exit", (code, signal) => {
        rejectMcpPending(entry, `MCP server '${id}' exited (code=${code ?? "null"}, signal=${signal ?? "null"}).`);
        if (mcpServers.get(id) === entry) {
            mcpServers.delete(id);
        }
    });
...
function dispatchMcpLine(id, entry, line) {
    let message;
    try {
        message = JSON.parse(line);
    } catch (error) {
        console.error(`[askk-mcp:${id}] dropped non-JSON stdout line: ${error.message}`);
        return;
    }
    const messageId = message?.id;
    if (messageId != null && entry.pending.has(messageId)) {
        const waiter = entry.pending.get(messageId);
        entry.pending.delete(messageId);
        clearTimeout(waiter.timer);
        waiter.resolve(message);
        return;
    }
    // Notification or unmatched response: keep a small ring buffer, never block.
    entry.notifications.push(message);
    if (entry.notifications.length > MCP_NOTIFICATION_RING) {
        entry.notifications.shift();
    }
}

export async function mcpSend(body, execConfig = {}) {
    const id = requiredString(body, "id");
    const message = body?.message;
    if (message == null || typeof message !== "object") {
        return { success: false, error: "mcp send requires a JSON-RPC 'message' object." };
    }

    const entry = mcpServers.get(id);
    if (!entry) {
        return { success: false, error: `mcp send: no live MCP server with id '${id}'.` };
    }
    ...
    try {
        ...
        return { success: false, error: `mcp send could not serialize message: ${error.message}` };
    }

    const messageId = message.id;
    if (messageId == null) {
        // Notification: write and return without awaiting a correlated response.
        if (!writeToChild(entry, serialized)) {
            return { success: false, error: `mcp send: stdin for '${id}' is not writable.` };
        }
    ...
    if (entry.pending.has(messageId)) {
        return {
            success: false,
            error: `mcp send: a request with id '${messageId}' is already in flight for '${id}'.`,
        };
    }

    const timeoutMs = clampInt(execConfig.execTimeoutMs ?? MCP_DEFAULT_TIMEOUT_MS, 1_000, 600_000);
    const responsePromise = new Promise((resolve) => {
        const timer = setTimeout(() => {
            if (entry.pending.get(messageId)?.timer === timer) {
                entry.pending.delete(messageId);
            }
            resolve({
                success: false,
                error: `mcp send: timed out after ${timeoutMs}ms awaiting response id '${messageId}' from '${id}'.`,
            });
        }, timeoutMs);
        entry.pending.set(messageId, {
            resolve: (jsonRpcResponse) => resolve({ success: true, data: jsonRpcResponse }),
            timer,
        });
    });
...
function writeToChild(entry, serialized) {
...
export function mcpKill(body) {
    const id = requiredString(body, "id");
    const entry = mcpServers.get(id);
    if (!entry) {
        return { success: false, error: `mcp kill: no live MCP server with id '${id}'.` };
    }
    teardownMcpServer(id, entry, "killed by request");
    return { success: true };
}

function teardownMcpServer(id, entry, reason) {
    if (mcpServers.get(id) === entry) {
        mcpServers.delete(id);
    }
    rejectMcpPending(entry, `MCP server '${id}' ${reason}.`);
    try {
        // SIGKILL guarantees the child is reaped; we don't need a graceful
        // drain because stdin/pending have already been torn down above.
...
function rejectMcpPending(entry, message) {
    for (const [messageId, waiter] of entry.pending) {
        clearTimeout(waiter.timer);
        waiter.resolve({ success: false, error: message });
        entry.pending.delete(messageId);
    }
}

export function killAllMcpServers() {
    for (const [id, entry] of mcpServers) {
        teardownMcpServer(id, entry, "bridge shutting down");
    }
}
...
Options:
  ...
  --allow-exec  Enable run_command and process-backed MCP servers so the agent can
                run bun/node/etc. and spawn stdio MCP servers in the run root.
...
Process-backed MCP relay (requires --allow-exec):
  POST /askk/mcp/spawn  { id, command, args?, env?, cwd? }  spawn a stdio MCP server
  POST /askk/mcp/send   { id, message }                     relay a JSON-RPC frame
  POST /askk/mcp/kill   { id }                              terminate the server
  Smoke-test the relay against scripts/mcp-stdio-stub.mjs (no Chrome needed); see
  the "Process-backed servers" section of the README for the full curl sequence.
...
if (process.argv[1] && fileURLToPath(import.meta.url) === process.argv[1]) {
    ...
    process.once("exit", killAllMcpServers);
