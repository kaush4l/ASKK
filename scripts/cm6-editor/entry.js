...
//
//   window.AskkCM.mount(hostId, { onChange(path, text), onSave(path, text) }, options?)
//       -> token (0 if the host element is missing)
//       // options: { fold?: boolean }  (code-folding gutter; default true)
...
//                {id, method:"format", path, text}   (on save; optional)
//   from worker: {id, result:{items:[{label, detail?, insertText?, kind?}]}}
//                {id, result:{contents}}
//                {id, result:{text}}                 // format: full formatted doc
//                {method:"publishDiagnostics", path, diagnostics:[...]}
//
// Format-on-save: on Mod-S the editor asks every attached service that handles
// the open doc's language for a `format` result before reporting the save. A
// service that does not implement formatting replies with an empty/non-`text`
// result (workers fall through to `{items:[]}` for unknown methods), so the
// document is saved unchanged. The first service that returns a `text` string
// wins; the formatted text replaces the buffer (cursor/scroll preserved) and
// is the value handed to `onSave`.
//
...
// Folding: `basicSetup` already wires the fold gutter, the fold keymap
// (Ctrl-Shift-[/] to fold/unfold, Ctrl-Alt-[/] to fold/unfold all) and the
// fold state. We import `codeFolding` only to override the folded-region
// placeholder; its `foldState` StateField is identity-deduped against
// basicSetup's, and its `foldConfig` is a Facet whose values merge, so adding
// it here is additive (no duplicate state, no second gutter).
...
const askkTheme = EditorView.theme(
  {
    ...
    // Fold gutter markers (the ►/▼ chevrons) and the folded-region
    // placeholder, re-themed for the ASKK dark palette — the CM6 base theme
    // styles the placeholder for a light background.
    ".cm-foldGutter span": {
      color: "#564b6e",
      cursor: "pointer",
    },
    ".cm-foldGutter span:hover": { color: "#a99cc4" },
    ".cm-foldPlaceholder": {
...
// EditorView.theme that hides the fold gutter. Held in the per-editor `fold`
// compartment so mount({ fold: false }) can suppress the gutter that
// `basicSetup` always installs, without rebuilding the editor.
const hideFoldGutterTheme = EditorView.theme({
...
// Ask the attached services to format the open doc. Services are queried in
// parallel; the first (in service order) that returns a `text` string wins.
// Returns the formatted text, or null when no service formats this language
// (services reply with an empty / non-`text` result for unknown methods, so
// "no formatter" naturally yields null and the save proceeds unchanged).
// Never rejects — `serviceRequest` resolves null on timeout / teardown.
function requestFormat(record) {
  return (async () => {
    const services = activeServices(record);
    if (!services.length) return null;
    const path = record.path;
    ...
    const results = await Promise.all(
      services.map((s) => serviceRequest(s, "format", { path, text })),
    );
    // The doc may have switched (tab change) while formatting was in flight;
    // a stale result must not clobber the now-current buffer.
    if (record.path !== path || !editors.has(record.hostId)) return null;
    for (const result of results) {
...
// Format-then-save: run the formatter, apply any result to the buffer, then
// report the saved text to `onSave`. The whole-document replace is flagged
// `silent` so it does not fire `onChange` / a redundant didChange. The cursor
// (primary-selection head) is clamped into the formatted text and restored so
// a save does not jump the caret to the end of the file. On any failure or
// "no formatter", the current text is saved unchanged. The path is captured up
// front so a tab switch mid-format can neither misattribute the save nor apply
// formatting to the wrong doc; everything after the single `await` runs
// synchronously, so no edit can interleave between the guard and the dispatch.
async function formatThenSave(record) {
  const cb = record.callbacks.onSave;
  const path = record.path;
  let formatted = null;
  try {
    formatted = await requestFormat(record);
  } catch (_err) {
    formatted = null;
  }
  // Re-check liveness/identity: the editor may have unmounted or switched docs
  // while awaiting the formatter.
  const live = editors.get(record.hostId);
  if (live !== record || record.path !== path) return;
  let textToSave = record.view.state.doc.toString();
  if (formatted !== null && formatted !== textToSave) {
    // Preserve the caret: clamp the old head offset into the new length. (A
    // bare full-document replace would otherwise leave the cursor at the end.)
    const head = Math.min(record.view.state.selection.main.head, formatted.length);
    record.silent = true;
    record.view.dispatch({
      changes: { from: 0, to: record.view.state.doc.length, insert: formatted },
      selection: { anchor: head },
    });
    record.silent = false;
    // The doc now equals `formatted` exactly; avoid the redundant toString().
    textToSave = formatted;
  }
  if (cb) cb(path, textToSave);
}
...
function extensionsFor(record, path) {
  return [
    basicSetup,
    // Customize the folded-region placeholder (basicSetup already provides the
    // gutter + fold state; this merges placeholder text into its fold config).
    codeFolding({ placeholderText: "⋯" }),
    keymap.of([
      {
        key: "Mod-s",
        run: () => {
          // Fire-and-forget: formatThenSave awaits the formatter, applies it,
          // then calls onSave. The keymap returns synchronously so CM6 treats
          // Mod-s as handled (preventing the browser's Save dialog).
          formatThenSave(record);
          return true;
        },
      },
      indentWithTab,
    ...
    oneDark,
    askkTheme,
    // Fold-gutter visibility toggle (mount option `fold`); empty when folding
    // is enabled, hides basicSetup's gutter when disabled. Initialized from
    // record.foldExt so the preference survives open()'s state rebuild.
    record.fold.of(record.foldExt),
...
const api = {
  ...
  // `options.fold` (default true) controls the code-folding gutter.
  mount(hostId, callbacks, options) {
    ...
    const record = {
      ...
      // Holds the fold-gutter visibility extension; persisted on the record so
      // it survives the state rebuild in open(). `[]` keeps basicSetup's
      // gutter; the hide-theme suppresses it when fold is disabled.
      fold: new Compartment(),
      foldExt: opts.fold === false ? hideFoldGutterTheme : [],
      path: "",
      silent: false,
      callbacks: callbacks || {},
      token: ++mountCounter,
